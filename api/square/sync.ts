...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!hasAccess) {
      console.error("[square/sync] Access check failed:", {
        userId: authResult.user.id,
        org_id,
      });
      return res
        .status(403)
        .json({ error: "Forbidden — not a member of this organisation" });
    }
    ...
    if (connErr || !conn) {
      return res
        .status(404)
        .json({ error: "No active Square connection found" });
    }
    ...
    if (!locationMaps || locationMaps.length === 0) {
      return res
        .status(400)
        .json({ error: "No active location mappings found" });
    }

    const locationToVenue = new Map(
      locationMaps.map((m) => [m.pos_location_id, m.venue_id]),
    );
    ...
    if (allOrders.length > 0) {
      const mappedOrders = allOrders.filter((order) =>
        locationToVenue.has(order.location_id),
      );
      ...
      for (let i = 0; i < orderRows.length; i += BATCH_SIZE) {
        ...
        const { error: upsertErr } = await db.from("orders").upsert(batch, {
      ...
      const trackingRows = mappedOrders.map((order) => {
        ...
        const itemCount =
          order.line_items?.reduce(
            (n, li) => n + parseInt(li.quantity || "0", 10),
            0,
          ) ?? 0;
        const paymentMethod = mapSquareTender(order.tenders?.[0]?.type);
        const isRefund = (order.refunds?.length ?? 0) > 0;
      ...
      for (let i = 0; i < trackingRows.length; i += BATCH_SIZE) {
        ...
        await db
          ...
          .then(({ error }) => {
            if (error)
              console.error("[square/sync] Tracking upsert error:", error);
          });
      }
    }
    ...
    try {
      ...
      if (org_id) {
        await db
          .from("pos_connections")
          .update({
            last_sync_status: `error: ${(err as Error).message?.slice(0, 200)}`,
          })
          .eq("org_id", org_id)
          .eq("provider", "square");
      }
    ...
    return res
      .status(500)
      .json({ error: (err as Error).message ?? "Sync failed" });
  }
}
