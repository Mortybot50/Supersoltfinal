...
async function processRefundReversal(
  ...
  if (!queueItem) {
    ...
    console.info(`[square/webhook] Reversal skipped — order ${squareOrderId} not in queue`)
    return
  }

  if (queueItem.reversed_at) {
    // Already reversed — idempotent
    console.info(`[square/webhook] Reversal already applied for order ${squareOrderId}`)
    return
  }
  ...
  if (queueItem.status !== 'completed') {
    ...
    console.info(`[square/webhook] Skipped unprocessed queue item for order ${squareOrderId} (reason: ${reason})`)
    return
  }
  ...
  if (movements.length === 0) {
    console.info(`[square/webhook] No depletion movements found for order ${squareOrderId}`)
    await (db
      .from('stock_depletion_queue' as 'pos_connections')
      .update({
        reversed_at: new Date().toISOString(),
  ...
  const { error: insertErr } = await (db
    .from('stock_movements' as 'pos_connections')
    .insert(reversals as unknown as Record<string, unknown>[]) as unknown as Promise<{
      error: unknown
    }>)
  ...
  console.info(
    ...
    (fraction < 1.0 ? ` (${Math.round(fraction * 100)}% partial)` : ' (full)')
  )
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (eventType === 'order.updated') {
      try {
        const order = event?.data?.object?.order as Record<string, unknown> | undefined
          ...
          if (conn) {
            if (orderState === 'CANCELED' || orderState === 'VOIDED') {
              ...
              if (items.length > 0) {
                ...
                console.info(`[square/webhook] Re-enqueued updated order ${orderId}`)
              }
            }
          }
        }
      } catch (orderUpdateErr) {
        console.error('[square/webhook] Failed to process order.updated:', orderUpdateErr)
      }
    }
    ...
    if (eventType === 'refund.created') {
      try {
        ...
        const refundAmount = (refund?.amount_money as Record<string, unknown> | undefined)?.amount as number | undefined
        ...
        if (orderId && locationId) {
          ...
          if (conn) {
            ...
            const orderTotal = (event?.data?.object?.order as Record<string, unknown> | undefined)
        ...
        console.error('[square/webhook] Failed to process refund reversal:', refundErr)
      }
    }
