...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  switch (action) {
    ...
    case "get-movements":
      return handleGetMovements(req, res);
    case "get-stock-levels":
      return handleGetStockLevels(req, res);
    case "get-forecast":
      return handleGetForecast(req, res);
    case "run-forecast":
      return handleRunForecast(req, res);
    case "update-forecast-accuracy":
      return handleUpdateForecastAccuracy(req, res);
    case "get-recommendations":
      return handleGetRecommendations(req, res);
    case "sync-catalog":
      return handleSyncCatalog(req, res);
    case "get-catalog-mappings":
      return handleGetCatalogMappings(req, res);
    case "save-catalog-mapping":
      return handleSaveCatalogMapping(req, res);
    case "save-modifier-mapping":
      return handleSaveModifierMapping(req, res);
    case "save-waste-factor":
      return handleSaveWasteFactor(req, res);
    default:
...
async function handleProcessQueue(req: VercelRequest, res: VercelResponse) {
  ...
  const { data: rawMappings } = await (db
    ...
    .select(
      "square_catalog_item_id, recipe_id, recipes(id, recipe_ingredients(ingredient_id, quantity, unit, ingredients(id, unit_cost)))",
    )
    .eq("org_id", orgId)
    .eq("is_active", true)
  ...
  if (allModifierIds.length > 0) {
    const { data: rawModifiers } = await (db
      ...
      .select(
        "square_modifier_id, ingredient_id, quantity_adjustment, adjustment_type, unit",
      )
      .eq("org_id", orgId)
  ...
  for (const item of items) {
    ...
    try {
      for (const lineItem of item.line_items) {
        ...
        for (const ri of recipeIngredients) {
          ...
          for (const mod of lineItem.modifiers) {
            ...
            if (!modMapping || modMapping.ingredient_id !== ri.ingredient_id)
              continue;

            if (modMapping.adjustment_type === "add") {
              depletionQty +=
                modMapping.quantity_adjustment * lineItem.quantity;
      ...
      console.error(
        `[inventory/process-queue] Failed item ${item.id}:`,
        errMsg,
      );
  ...
  return res
    .status(200)
    .json({ processed, failed, total_movements: totalMoves, results });
}
...
async function handleGetQueue(req: VercelRequest, res: VercelResponse) {
  ...
  const query = db
    .from("stock_depletion_queue" as unknown as "pos_connections")
    .select(
      "id, square_order_id, status, error_message, retry_count, processed_at, created_at",
    )
    .eq("org_id", orgId)
  ...
  if (venueId) {
    (query as unknown as { eq: (col: string, val: string) => unknown }).eq(
      "venue_id",
      venueId,
    );
  }
...
async function handleGetMovements(req: VercelRequest, res: VercelResponse) {
  ...
  let queryBuilder = db
    .from("stock_movements" as unknown as "pos_connections")
    .select(
      "id, ingredient_id, movement_type, quantity, unit, unit_cost, reference_type, reference_id, notes, created_by, created_at",
    )
    .eq("org_id", orgId)
    .eq("venue_id", venueId)
    .order("created_at", { ascending: false })
...
async function handleGetStockLevels(req: VercelRequest, res: VercelResponse) {
  ...
  const { data: recentMovements, error: movErr } = await (db
    ...
    data:
      | { ingredient_id: string; quantity: number; created_at: string }[]
      | null;
    error: unknown;
  }>);
    ...
    (ingredients ?? []).map(async (ing) => {
      ...
      try {
        const { data: stockData } = (await db.rpc(
          ...
          { p_ingredient_id: ing.id, p_venue_id: venueId } as unknown as Record<
            string,
            never
          >,
      ...
      const daysRemaining =
        avgDailyUsage > 0 ? currentStock / avgDailyUsage : null;
...
async function handleGetForecast(req: VercelRequest, res: VercelResponse) {
  ...
  let queryBuilder = db
    ...
    .select(
      "id, menu_item_id, forecast_date, predicted_quantity, confidence_lower, confidence_upper, model_version, mape, created_at",
    )
    .eq("org_id", orgId)
    .eq("venue_id", venueId)
...
async function handleRunForecast(req: VercelRequest, res: VercelResponse) {
  ...
  const body = req.body as
    | { org_id?: string; venue_id?: string; horizon_days?: number }
    | undefined;
  const orgId = body?.org_id;
  const venueId = body?.venue_id;
  ...
  if (movErr) {
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch historical movements" });
  }
  ...
  for (const menuItem of menuItems ?? []) {
    const hw = holtwinters(dailySeries, {
      alpha: 0.3,
      beta: 0.1,
      gamma: 0.2,
      season: 7,
    ...
    if (dailySeries.length >= 7) {
      ...
      mape =
        Math.round(
          (mapeSum / actuals.filter((a) => a > 0).length) * 100 * 1000,
        ) / 1000;
    }
  ...
  if (forecastRows.length === 0) {
    return res
      .status(200)
      .json({ forecasted: 0, message: "No forecast rows generated" });
  }
...
async function handleGetRecommendations(
  ...
  const { data: ingredients, error: ingErr } = await (db
    .from("ingredients" as unknown as "pos_connections")
    .select(
      "id, name, unit, unit_cost, reorder_point, par_level, pack_size, supplier_id, suppliers(id, name, lead_time_days)",
    )
    .eq("org_id", orgId) as unknown as Promise<{
    data:
      | {
  ...
  const { data: leadTimeLogs } = await (db
    ...
    data:
      | { supplier_id: string; actual_lead_days: number; received_at: string }[]
      | null;
  }>);
  ...
  const recommendations = await Promise.all(
    (ingredients ?? []).map(async (ing) => {
      ...
      const dayMap =
        usageByIngredientDay.get(ing.id) ?? new Map<string, number>();
      const dayValues = [...dayMap.values()];

      const avgDailyUsage =
        dayValues.length > 0 ? dayValues.reduce((s, v) => s + v, 0) / 14 : 0;

      if (avgDailyUsage <= 0) return null;

      // Std deviation of daily demand
      const mean =
        dayValues.reduce((s, v) => s + v, 0) / Math.max(dayValues.length, 1);
      const variance =
        dayValues.reduce((s, v) => s + Math.pow(v - mean, 2), 0) /
        Math.max(dayValues.length, 1);
      const stdDev = Math.sqrt(variance);

      const rawSupplier = ing.suppliers as unknown;
      const supplierId =
        (rawSupplier && typeof rawSupplier === "object" && "id" in rawSupplier
      ...
      const staticLeadTime =
        (rawSupplier &&
        typeof rawSupplier === "object" &&
        "lead_time_days" in rawSupplier
          ? (rawSupplier as { lead_time_days: number | null }).lead_time_days
          : null) ?? 3;
      const learnedLeadTime = supplierId
        ? learnedLeadTimeBySupplier.get(supplierId)
        : undefined;
      const learnedSampleCount = supplierId
        ? (logsBySupplier.get(supplierId)?.length ?? 0)
        : 0;
      // Prefer learned lead time once we have ≥ 3 deliveries for statistical validity
      const leadTimeDays =
        learnedLeadTime !== undefined && learnedSampleCount >= 3
          ? learnedLeadTime
          : staticLeadTime;
      ...
      const supplierName =
        rawSupplier && typeof rawSupplier === "object" && "name" in rawSupplier
      ...
      return {
        ...
        pack_size: packSize,
        estimated_cost: ing.unit_cost
          ? Math.round(orderQty * ing.unit_cost * 100) / 100
          : null,
        urgency,
...
async function handleSyncCatalog(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to decrypt Square access token" });
  }
  ...
  if (allItems.length === 0) {
    return res
      .status(200)
      .json({ synced: 0, message: "No catalog items found" });
  }
...
async function handleGetCatalogMappings(
  req: VercelRequest,
  res: VercelResponse,
) {
  if (req.method !== "GET")
  ...
  let queryBuilder = db
    .from("square_catalog_mappings" as unknown as "pos_connections")
    .select(
      "id, square_catalog_item_id, square_item_name, recipe_id, venue_id, is_active, created_at, updated_at, recipes(id, name)",
    )
    .eq("org_id", orgId)
...
async function handleSaveCatalogMapping(
  req: VercelRequest,
  res: VercelResponse,
) {
  if (req.method !== "POST")
  ...
  const orgId = body?.org_id;
  if (!orgId) return res.status(400).json({ error: "org_id required" });
  if (!body?.square_catalog_item_id)
    return res.status(400).json({ error: "square_catalog_item_id required" });
...
async function handleSaveModifierMapping(
  req: VercelRequest,
  res: VercelResponse,
) {
  if (req.method !== "POST")
  ...
  const orgId = body?.org_id;
  if (!orgId) return res.status(400).json({ error: "org_id required" });
  if (!body?.square_modifier_id)
    return res.status(400).json({ error: "square_modifier_id required" });
  ...
  if (!["add", "remove", "replace"].includes(adjustmentType)) {
    return res
      .status(400)
      .json({ error: "adjustment_type must be add|remove|replace" });
  }
    ...
    .upsert(
      {
        ...
        square_modifier_name:
          body.square_modifier_name ?? body.square_modifier_id,
...
async function handleSaveWasteFactor(req: VercelRequest, res: VercelResponse) {
  ...
  if (wastePct < 0 || wastePct > 100) {
    return res
      .status(400)
      .json({ error: "waste_percentage must be between 0 and 100" });
  }

  const wasteType = body.waste_type ?? "trim";
  if (
    !["trim", "spillage", "evaporation", "overportioning"].includes(wasteType)
  ) {
    return res
      .status(400)
      .json({
        error: "waste_type must be trim|spillage|evaporation|overportioning",
      });
  }
...
async function handleUpdateForecastAccuracy(
  req: VercelRequest,
  res: VercelResponse,
) {
  if (req.method !== "POST")
  ...
  const body = req.body as
    | { org_id?: string; venue_id?: string; days_back?: number }
    | undefined;
  const orgId = body?.org_id;
  const venueId = body?.venue_id;
  ...
  if (forecastErr) {
    console.error(
      "[inventory/update-forecast-accuracy] Forecast fetch error:",
      forecastErr,
    );
    return res.status(500).json({ error: "Failed to fetch forecasts" });
  }
  ...
  if (!forecasts || forecasts.length === 0) {
    return res
      .status(200)
      .json({ updated: 0, message: "No past forecasts to evaluate" });
  }
  ...
  const { data: catalogMappings } = await (db
    ...
    data:
      | {
          recipe_id: string | null;
          recipes: { menu_item_id: string | null } | null;
        }[]
      | null;
  }>);
  ...
  let updatedCount = 0;
  const mapeByMenuItem = new Map<
    string,
    { errors: number[]; predicted: number[]; actual: number[] }
  >();
  ...
  for (const [menuItemId, data] of mapeByMenuItem.entries()) {
    ...
    if (validActuals.length > 0) {
      const mapeSum = data.errors
        .slice(0, validActuals.length)
        .reduce((s, e) => s + e, 0);
      itemMape =
        Math.round(
          (mapeSum / validActuals.reduce((s, a) => s + a, 0)) * 100 * 1000,
        ) / 1000;
    }
    itemSummaries.push({
