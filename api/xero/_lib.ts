...
export function signState(payload: Record<string, string>): string {
  ...
  const sig = createHmac('sha256', env('XERO_CLIENT_SECRET')).update(b64).digest('hex')
  return `${b64}.${sig}`
}
...
export function verifyState(signed: string): Record<string, string> {
  ...
  const expected = createHmac('sha256', env('XERO_CLIENT_SECRET')).update(b64).digest('hex')
  if (sig !== expected) throw new Error('State signature mismatch — possible CSRF')
...
export async function checkOrgAccess(accessToken: string, orgId: string): Promise<boolean> {
...
export const DEFAULT_AU_ACCOUNT_MAPPINGS: Array<{
  ...
  { supersolt_category: 'revenue_food',       xero_account_code: '200', xero_account_name: 'Sales' },
  { supersolt_category: 'revenue_beverage',   xero_account_code: '200', xero_account_name: 'Sales' },
  { supersolt_category: 'cogs_food',          xero_account_code: '310', xero_account_name: 'Cost of Goods Sold' },
  { supersolt_category: 'cogs_beverage',      xero_account_code: '310', xero_account_name: 'Cost of Goods Sold' },
  { supersolt_category: 'labour_wages',       xero_account_code: '477', xero_account_name: 'Wages and Salaries' },
  { supersolt_category: 'labour_super',       xero_account_code: '478', xero_account_name: 'Superannuation' },
  { supersolt_category: 'gst_collected',      xero_account_code: '820', xero_account_name: 'GST' },
  { supersolt_category: 'gst_paid',           xero_account_code: '820', xero_account_name: 'GST' },
  { supersolt_category: 'overhead_rent',      xero_account_code: '493', xero_account_name: 'Rent' },
