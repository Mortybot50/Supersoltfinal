...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!tenantsRes.ok) {
      console.error(
        "[xero/callback] Failed to fetch tenants:",
        await tenantsRes.text(),
      );
    ...
    const { data: existingMappings } = (await db
      ...
      .eq("org_id", orgId)) as {
      data: Array<{ supersolt_category: string }> | null;
    };

    const existingCategories = new Set(
      (existingMappings ?? []).map((m) => m.supersolt_category),
    );
    const toInsert = DEFAULT_AU_ACCOUNT_MAPPINGS.filter(
      (m) => !existingCategories.has(m.supersolt_category),
