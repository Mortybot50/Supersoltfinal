  ...
  switch (action) {
    ...
    default:
      return res
        .status(400)
        .json({
          error: "Invalid action. Use ?action=sync|accounts|disconnect",
        });
  }
}
...
async function handleAccounts(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const fetchAccounts = async (at: string) =>
      fetch(
        `${XERO_API_BASE}/api.xro/2.0/Accounts?where=Status%3D%3D%22ACTIVE%22`,
        {
          headers: {
  ...
  try {
    ...
    if (conn.refresh_token) {
      try {
        ...
        console.warn(
          "[xero/disconnect] Token revocation failed (continuing):",
          revokeErr,
        );
      }
    }
...
async function handleSync(req: VercelRequest, res: VercelResponse) {
  ...
  const xeroFetch = async (path: string, options: RequestInit = {}) => {
    const makeReq = () =>
      fetch(`${XERO_API_BASE}/api.xro/2.0/${path}`, {
        ...options,
        headers: {
    ...
    try {
      ...
      if (orders && orders.length > 0) {
        ...
        const byDate = orders.reduce<
          Record<string, { total: number; tax: number }>
        >((acc, o) => {
      ...
      errors.push(
        `Sales sync error: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  if (syncTypes.includes("purchases")) {
    try {
      const poQuery = db
        ...
        .select(
          "id, created_at, supplier_id, total_amount, status, suppliers(name)",
        )
        .eq("org_id", orgId)
        ...
        for (const po of pos) {
          ...
          const supplierName =
            (Array.isArray(rawSupplier)
              ? (rawSupplier[0] as { name: string })?.name
              : (rawSupplier as { name: string } | null)?.name) ?? "Supplier";
          const billPayload = {
            Invoices: [
              {
                Type: "ACCPAY",
      ...
      errors.push(
        `Purchases sync error: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  if (syncTypes.includes("payroll")) {
    try {
      ...
      if (timesheets && timesheets.length > 0) {
        const totalWages = timesheets.reduce(
          (s, t) => s + Number(t.gross_pay ?? 0),
          0,
        );
      ...
      errors.push(
        `Payroll sync error: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  const syncStatus =
    errors.length === 0 ? "success" : totalPushed > 0 ? "partial" : "error";
  ...
  await db
    ...
    .update({
      ...
      last_sync_status:
        syncStatus === "success" ? "success" : `error: ${errorSummary}`,
    } as Record<string, unknown>)
    .eq("id", conn.id);
