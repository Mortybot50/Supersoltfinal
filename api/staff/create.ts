...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { data: { user: callingUser }, error: authError } = await db.auth.getUser(token)
  ...
  try {
    ...
    if (!callerMember || !['admin', 'owner'].includes(callerMember.role)) {
      return res.status(403).json({ error: 'Not authorised — must be org admin' })
    }

    // Generate a placeholder email if none provided
    const staffEmail = body.email || `staff-${crypto.randomUUID().slice(0, 8)}@placeholder.supersolt.local`
    const isPlaceholder = !body.email

    // 1. Create placeholder auth user (random password, no email confirmation)
    const { data: authUser, error: createUserError } = await db.auth.admin.createUser({
      email: staffEmail,
    ...
    if (createUserError || !authUser.user) {
      console.error('[staff/create] Failed to create auth user:', createUserError)
    ...
    if (orgMemberError || !orgMember) {
      console.error('[staff/create] Failed to create org_member:', orgMemberError)
      await db.auth.admin.deleteUser(userId)
    ...
    if (staffError || !staff) {
      console.error('[staff/create] Failed to create staff record:', staffError)
      // Cleanup chain
    ...
    if (body.venue_id) {
      const { error: venueAccessError } = await db
        .from('venue_access')
        .insert({
          org_member_id: orgMember.id,
      ...
      if (venueAccessError) {
        console.error('[staff/create] Failed to create venue_access (non-fatal):', venueAccessError)
