    ...
    try {
      ...
      if (data) {
        const formattedItems = data.map(item => ({
          ...
          section_id: (item as any).section_id ?? (item as any).menu_group ?? '',
          recipe_id: (item as any).recipe_id ?? '',
          recipe_name: (item as any).recipe_name ?? '',
          organization_id: (item as any).organization_id ?? (item as any).org_id ?? '',
          price_mode: (item as any).price_mode ?? 'MANUAL' as const,
          price: (item as any).price ?? item.selling_price ?? 0,
          gst_mode: (item as any).gst_mode ?? 'INC' as const,
          gst_rate_percent: (item as any).gst_rate_percent ?? 10,
          show_on_menu: (item as any).show_on_menu ?? item.active ?? true,
          display_order: (item as any).display_order ?? 0,
          tags: (item as any).tags ?? [],
          allergens: (item as any).allergens ?? [],
          cost_per_serve: (item as any).cost_per_serve ?? item.cost_price ?? 0,
          gp_target_percent: (item as any).gp_target_percent ?? 65,
          effective_price: (item as any).effective_price ?? item.selling_price ?? 0,
