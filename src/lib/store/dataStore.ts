...
interface MenuItemRow extends Omit<
  Types.MenuItem,
...
interface PurchaseOrderRow extends Omit<
  Types.PurchaseOrder,
  | "order_date"
...
interface DataState {
  ...
  updateIngredient: (
    id: string,
    updates: Partial<Types.Ingredient>,
  ) => Promise<void>;
  ...
  updateSupplier: (
    id: string,
    updates: Partial<Types.Supplier>,
  ) => Promise<void>;
  ...
  addPurchaseOrder: (
    po: Types.PurchaseOrder,
    items: Types.PurchaseOrderItem[],
  ) => Promise<void>;
  updatePurchaseOrder: (
    id: string,
    updates: Partial<Types.PurchaseOrder>,
  ) => Promise<void>;
  ...
  updateStockCount: (
    id: string,
    updates: Partial<Types.StockCount>,
  ) => Promise<void>;
  ...
  updateWasteEntry: (
    id: string,
    updates: Partial<Types.WasteEntry>,
  ) => Promise<void>;
  ...
  updateMenuItem: (
    id: string,
    updates: Partial<Types.MenuItem>,
  ) => Promise<void>;
  ...
  updateMenuSection: (
    id: string,
    updates: Partial<Types.MenuSection>,
  ) => Promise<void>;
  ...
  updateRecipeIngredient: (
    id: string,
    updates: Partial<Types.RecipeIngredient>,
  ) => Promise<void>;
  ...
  updateOnboardingStep: (
    stepId: string,
    updates: Partial<Types.OnboardingStep>,
  ) => void;
  ...
  updateOnboardingDocument: (
    docId: string,
    updates: Partial<Types.OnboardingDocument>,
  ) => void;
  updateStaffOnboarding: (
    staffId: string,
    updates: Partial<Types.Staff>,
  ) => void;
  ...
  updateShiftTemplate: (
    id: string,
    updates: Partial<Types.ShiftTemplate>,
  ) => void;
  ...
  updateStaffAvailability: (
    id: string,
    updates: Partial<Types.StaffAvailability>,
  ) => void;
  deleteStaffAvailability: (id: string) => void;
  getStaffAvailabilityForWeek: (
    staffId: string,
    weekStart: Date,
  ) => Types.StaffAvailability[];
  ...
  createOpenShift: (
    shift: Omit<Types.RosterShift, "id" | "staff_id" | "staff_name">,
  ) => void;
  ...
  updateInvoiceLineItem: (
    id: string,
    updates: Partial<Types.InvoiceLineItem>,
  ) => Promise<void>;
  addReconciliation: (
    log: Types.ReconciliationLog,
    lineItems: Types.ReconciliationLineItem[],
  ) => Promise<void>;
    ...
    (set, get) => ({
      ...
      deleteStaff: (id) =>
        set((state) => ({
          ...
          rosterShifts: state.rosterShifts.filter(
            (shift) => shift.staff_id !== id,
          ),
        })),

      setRosterShifts: (rosterShifts) => set({ rosterShifts }),
      addRosterShift: (shift) =>
        set((state) => ({ rosterShifts: [...state.rosterShifts, shift] })),
      updateRosterShift: (id, updates) =>
        set((state) => ({
          rosterShifts: state.rosterShifts.map((s) =>
            s.id === id ? { ...s, ...updates } : s,
          ),
        })),
      deleteRosterShift: (id) =>
        set((state) => ({
      ...
      copyPreviousWeekRoster: (targetWeekStart) => {
        ...
        const prevWeekShifts = state.rosterShifts.filter((s) => {
          ...
          return (
            shiftDate >= prevWeekStart &&
            shiftDate <= prevWeekEnd &&
            s.status !== "cancelled"
          );
        });
        ...
        set((state) => ({
          rosterShifts: [...state.rosterShifts, ...newShifts],
        }));
      },
        ...
        try {
          ...
          if (data) {
            ...
            const formattedOrders = data.map((order) => ({
              ...
              channel: order.channel as
                | "dine-in"
                | "takeaway"
                | "delivery"
                | "online",
              payment_method: order.payment_method as
                | "card"
                | "cash"
                | "digital_wallet",
            }));
      ...
      addIngredient: async (ingredient) => {
        try {
          ...
          const ingredientData: Omit<Types.Ingredient, "last_cost_update"> & {
            last_cost_update: string;
          } = {
            ...ingredient,
            last_cost_update:
              ingredient.last_cost_update instanceof Date
          ...
          if (data) {
            set((state) => ({
              ingredients: [...state.ingredients, data as Types.Ingredient],
            }));
          }
        } catch (error) {
      ...
      updateIngredient: async (id, updates) => {
        try {
          ...
          // Convert Date to ISO string for Supabase
          const updateData: Partial<Types.Ingredient> & {
            last_cost_update?: string;
          } = { ...updates };
          if (updates.last_cost_update instanceof Date) {
            updateData.last_cost_update =
              updates.last_cost_update.toISOString();
          }
      ...
      addSupplier: async (supplier) => {
        try {
          ...
          const dbRow = {
            ...
            min_order_value: supplier.minimum_order
              ? supplier.minimum_order / 100
              : undefined,
            is_active: supplier.active,
            notes: supplier.notes,
          };

          const { error } = await supabase.from("suppliers").insert([dbRow]);
      ...
      updateSupplier: async (id, updates) => {
        try {
          ...
          if (updates.organization_id !== undefined)
            dbUpdates.org_id = updates.organization_id;
          if (updates.contact_person !== undefined)
            dbUpdates.contact_name = updates.contact_person;
          if (updates.address !== undefined)
            dbUpdates.address_line1 = updates.address;
          if (updates.active !== undefined)
            dbUpdates.is_active = updates.active;
          if (updates.delivery_lead_days !== undefined)
            dbUpdates.lead_time_days = updates.delivery_lead_days;
          if (updates.minimum_order !== undefined)
            dbUpdates.min_order_value = updates.minimum_order / 100;
          if (updates.payment_terms !== undefined)
            dbUpdates.payment_terms = updates.payment_terms.replace(/-/g, "_");
        ...
        try {
          ...
          if (latestPO) {
            const poData: PurchaseOrderRow = {
              ...latestPO,
              order_date:
                latestPO.order_date instanceof Date
                  ? latestPO.order_date.toISOString()
                  : String(latestPO.order_date),
              expected_delivery_date:
                latestPO.expected_delivery_date instanceof Date
          ...
          console.error("Failed to save purchase order:", msg);
      ...
      loadStockCountsFromDB: async () => {
        try {
          ...
          if (countsData) {
            ...
            const itemsByCount = (itemsData || []).reduce(
              (acc, item) => {
      ...
      updateStockCount: async (
        ...
        try {
          ...
          const updateData: Partial<StockCountRow> = {
            ...updates,
          ...
          set((state) => {
            const updatedIngredients = state.ingredients.map((ingredient) => {
              const countItem = count.items?.find(
                (item: StockCountItem) => item.ingredient_id === ingredient.id,
              );
              if (countItem) {
                return {
                  ...ingredient,
            ...
            await supabase
              ...
              .update({
                current_stock: Math.max(
                  0,
                  ingredient.current_stock - waste.quantity,
                ),
              })
            ...
            const updatedIngredients = state.ingredients.map((i) =>
              i.id === waste.ingredient_id
                ? {
                    ...
                    current_stock: Math.max(
                      0,
                      i.current_stock - waste.quantity,
                    ),
                  }
                : i,
            );
      ...
      updateWasteEntry: async (
        ...
        try {
          ...
          const updateData: Partial<WasteLogRow> = {
            ...updates,
            waste_date: undefined,
          };
        ...
        try {
          ...
          if (data) {
            const formattedItems = data.map((item) => ({
              ...item,
              // Map DB columns to app type fields with safe defaults
              section_id: item.menu_group ?? "",
              recipe_id: "",
              recipe_name: "",
              organization_id: "",
              price_mode: "MANUAL" as const,
              price: item.selling_price ?? 0,
              gst_mode: "INC" as const,
              gst_rate_percent: 10,
              show_on_menu: item.active ?? true,
              display_order: 0,
              tags: [] as string[],
              allergens: [] as string[],
              cost_per_serve: item.cost_price ?? 0,
              gp_target_percent: 65,
              effective_price: item.selling_price ?? 0,
      ...
      updateMenuSection: async (id, updates) => {
        try {
          ...
          if (updates.display_order !== undefined)
            dbUpdates.sort_order = updates.display_order;
      ...
      reorderSections: (sections) => {
        set({
          menuSections: sections.map((s, i) => ({ ...s, display_order: i })),
        });
      },
      ...
      calculateMenuItemFields: (item) => {
        ...
        const effectivePrice =
          item.price_mode === "AUTO_FROM_RECIPE"
        ...
        if (item.gst_mode === "INC") {
          ...
          gstAmount = Math.round(
            (effectivePrice * item.gst_rate_percent) /
              (100 + item.gst_rate_percent),
          );
          priceExGst = effectivePrice - gstAmount;
        } else {
          // Price excludes GST: GST = price × (10 / 100)
          gstAmount = Math.round(
            (effectivePrice * item.gst_rate_percent) / 100,
          );
        ...
        return {
          ...
          std_drinks:
            stdDrinks > 0 ? Math.round(stdDrinks * 10) / 10 : undefined,
        };
      },
      ...
      calculateMenuAnalytics: () => {
        ...
        const menuRevenue = activeItems.reduce(
          (sum, i) => sum + (i.price_ex_gst || 0),
          0,
        );
        const menuCogs = activeItems.reduce(
          (sum, i) => sum + i.cost_per_serve,
          0,
        );
        const menuGpPercent =
          menuRevenue > 0 ? ((menuRevenue - menuCogs) / menuRevenue) * 100 : 0;
        ...
        activeItems.forEach((item) => {
          ...
          if (
            item.price_ex_gst !== undefined &&
            item.price_ex_gst < item.cost_per_serve
          ) {
            warnings.push({
      ...
      loadRecipesFromDB: async () => {
        try {
          const { loadRecipesFromDB } =
            await import("@/lib/services/recipeService");
        ...
        try {
          ...
          if (isNew) {
            set((state) => ({
              ...
              recipeIngredients: [
                ...state.recipeIngredients.filter(
        ...
        try {
          ...
          const { error } = await supabase.from("recipes").insert({
            ...
            method: JSON.stringify(
              recipe.steps.filter((s: string) => s.trim() !== ""),
            ),
      ...
      updateRecipe: async (id, updates) => {
        try {
          ...
          if (updates.serves !== undefined)
            dbUpdates.batch_yield = updates.serves;
          if (updates.wastage_percent !== undefined)
            dbUpdates.waste_percent = updates.wastage_percent;
          if (updates.gp_target_percent !== undefined)
            dbUpdates.gp_target_percent = updates.gp_target_percent;
          if (updates.instructions !== undefined)
            dbUpdates.description = updates.instructions || null;
          if (updates.steps !== undefined)
            dbUpdates.method = JSON.stringify(updates.steps);
          if (updates.allergens !== undefined)
            dbUpdates.allergens = updates.allergens;
          if (updates.status !== undefined) dbUpdates.status = updates.status;
          if (updates.total_cost !== undefined)
            dbUpdates.cost_per_batch = updates.total_cost;
          if (updates.cost_per_serve !== undefined)
            dbUpdates.cost_per_serve = updates.cost_per_serve;
          if (updates.suggested_price !== undefined)
            dbUpdates.suggested_price = updates.suggested_price;
          if (Object.keys(dbUpdates).length > 0) {
            const { error } = await supabase
              .from("recipes")
              .update(dbUpdates)
      ...
      updateRecipeIngredient: async (id, updates) => {
        try {
          ...
          if (updates.line_cost !== undefined)
            dbUpdates.cost = updates.line_cost;
          if (updates.product_id !== undefined)
            dbUpdates.ingredient_id = updates.product_id;
          if (Object.keys(dbUpdates).length > 0) {
            const { error } = await supabase
      ...
      getRecipeIngredients: (recipeId) => {
        return get().recipeIngredients.filter(
      ...
      recalculateRecipeCosts: (recipeId) => {
        ...
        const ingredients = get().recipeIngredients.filter(
          (ri) => ri.recipe_id === recipeId,
        );

        // Calculate total cost
        const totalCost = ingredients.reduce(
          (sum, ing) => sum + ing.line_cost,
          0,
        );
        ...
        try {
          ...
          if (data) {
            const formatted = data.map((s) => ({
              ...
              payment_terms: (s.payment_terms?.replace(/_/g, "-") ??
                "net-30") as Types.Supplier["payment_terms"],
              ...
              minimum_order: s.min_order_value
                ? Math.round(Number(s.min_order_value) * 100)
                : undefined,
              notes: s.notes,
      ...
      loadPurchaseOrdersFromDB: async () => {
        try {
          ...
          if (poData) {
            ...
            // Group items by PO
            const itemsByPO = (itemsData || []).reduce(
              (acc, item) => {
            ...
            const posWithItems = poData.map((po) => ({
              ...
              delivered_at: po.delivered_at
                ? new Date(po.delivered_at)
                : undefined,
              cancelled_at: po.cancelled_at
                ? new Date(po.cancelled_at)
                : undefined,
      ...
      updatePurchaseOrder: async (id, updates) => {
        try {
          ...
          // Prepare update data
          const updateData: Partial<PurchaseOrderRow> = {
            updated_at: new Date().toISOString(),
          };
      ...
      importOrders: async (orders, orderItems, tenders) => {
        try {
          ...
          if (orderItems.length > 0) {
            const { calculateDepletion, applyDepletionToIngredients } =
              await import("@/lib/services/inventoryDepletion");
            const state = get();
            ...
            if (result.depletions.length > 0) {
              const updatedIngredients = applyDepletionToIngredients(
                result,
                state.ingredients,
              );
        ...
        try {
          ...
          const newOrders = deduplicatedInput.map((po) => {
            ...
            return {
              ...
              channel: po.channel,
              payment_method: (po.payment_method || "card") as
                | "card"
                | "cash"
                | "digital_wallet",
        ...
        try {
          ...
          if (ingredients.length > 0) {
            const ingredientsData = ingredients.map((ing) => ({
              ...ing,
              last_cost_update:
                ing.last_cost_update instanceof Date
      ...
      updateOnboardingStep: (stepId, updates) =>
        set((state) => ({
      ...
      clearAllData: () => {
        ...
        if (confirmation === "DELETE ALL DATA") {
          ...
          import("sonner").then(({ toast }) => {
            toast.error(
              "Data clear cancelled - confirmation text did not match",
            );
          });
        }
      },
        ...
        try {
          ...
          if (invoiceData) {
            const { data: lineData, error: lineError } = await supabase
              ...
              .in(
                "invoice_id",
                invoiceData.map((i) => i.id),
              );

            if (lineError) throw lineError;

            const linesByInvoice = (lineData || []).reduce(
              (acc, l) => {
                if (!acc[l.invoice_id]) acc[l.invoice_id] = [];
                acc[l.invoice_id].push({
                  ...l,
                  ingredient_name: (l.ingredients as { name: string } | null)
                    ?.name,
                });
                return acc;
              },
            ...
            set({
              invoices: formatted,
              invoiceLineItems: (lineData as Types.InvoiceLineItem[]) || [],
            });
          }
        } catch (error) {
        ...
        try {
          ...
          if (lineItems.length > 0) {
            const rows = lineItems.map(
              ({ ingredient_name: _ing, ...rest }) => rest,
            );
      ...
      updateInvoiceLineItem: async (id, updates) => {
        try {
          ...
          const { ingredient_name: _ing, ...safeUpdates } =
            updates as Types.InvoiceLineItem;
          const { error } = await supabase
            .from("invoice_line_items")
            .update(safeUpdates)
            .eq("id", id);
      ...
      addReconciliation: async (log, lineItems) => {
        try {
          ...
          set((state) => ({
            reconciliationLogs: [
              { ...log, line_items: lineItems },
              ...state.reconciliationLogs,
            ],
          }));
      ...
      loadReconciliationsFromDB: async (venueId) => {
        try {
          ...
          if (data) {
            ...
            const linesByRecon = (lineData || []).reduce(
              (acc, l) => {
