...
export function useLabourCostReport(venueId: string | undefined, dateRange: DateRange) {
  ...
  const { data: raw, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('roster_shifts')
        .select('shift_date, start_time, end_time, break_duration_mins, base_cost, penalty_cost, estimated_cost, penalty_type, status')
        .eq('venue_id', venueId!)
        .gte('shift_date', from)
        .lte('shift_date', to)
  ...
  const data = useMemo((): LabourCostData => {
    if (!raw || raw.length === 0) {
      return { days: [], totalHours: 0, totalCost: 0, baseCost: 0, penaltyCost: 0, avgHourlyRate: 0, shiftCount: 0 }
    }
    ...
    const days = eachDayOfInterval({ start: dateRange.from, end: dateRange.to }).map(day => {
      ...
      const totalHours = dayShifts.reduce((sum, s) =>
        sum + calcHours(s.start_time, s.end_time, s.break_duration_mins || 0), 0)
      const baseCost = Math.round(dayShifts.reduce((sum, s) => sum + (s.base_cost || 0), 0) * 100)
      const penaltyCost = Math.round(dayShifts.reduce((sum, s) => sum + (s.penalty_cost || 0), 0) * 100)
      const totalCost = Math.round(dayShifts.reduce((sum, s) => sum + (s.estimated_cost || 0), 0) * 100)
      return { date: dateStr, label: format(day, 'EEE d'), baseCost, penaltyCost, totalCost, totalHours }
    })

    const totalHours = raw.reduce((sum, s) =>
      sum + calcHours(s.start_time, s.end_time, s.break_duration_mins || 0), 0)
    const totalCost = Math.round(raw.reduce((sum, s) => sum + (s.estimated_cost || 0), 0) * 100)
    const baseCost = Math.round(raw.reduce((sum, s) => sum + (s.base_cost || 0), 0) * 100)
    const penaltyCost = Math.round(raw.reduce((sum, s) => sum + (s.penalty_cost || 0), 0) * 100)
    const avgHourlyRate = totalHours > 0 ? totalCost / totalHours / 100 : 0

    return { days, totalHours, totalCost, baseCost, penaltyCost, avgHourlyRate, shiftCount: raw.length }
  }, [raw, dateRange])
...
export function useLabourPercentReport(venueId: string | undefined, dateRange: DateRange) {
  const from = format(dateRange.from, 'yyyy-MM-dd')
  const to = format(dateRange.to, 'yyyy-MM-dd')
  ...
  const data = useMemo((): LabourPercentData => {
    const days = eachDayOfInterval({ start: dateRange.from, end: dateRange.to }).map(day => {
      ...
      const labourCost = Math.round(dayShifts.reduce((sum, s) => sum + (s.estimated_cost || 0), 0) * 100)

      const dayOrders = (orders || []).filter(o => o.order_datetime?.startsWith(dateStr))
      const revenue = dayOrders
        .filter(o => !o.is_void)
        .reduce((sum, o) => sum + (o.is_refund ? -(o.net_amount || 0) : (o.net_amount || 0)), 0)

      const labourPercent = revenue > 0 ? (labourCost / revenue) * 100 : 0

      return { date: dateStr, label: format(day, 'EEE d'), revenue, labourCost, labourPercent }
    })
    ...
    return { rows: days, totalRevenue, totalLabourCost, labourPercent, hasRevenue: totalRevenue > 0 }
...
export function useRosteredVsActualReport(venueId: string | undefined, dateRange: DateRange) {
  const from = format(dateRange.from, 'yyyy-MM-dd')
  const to = format(dateRange.to, 'yyyy-MM-dd')
  ...
  const data = useMemo((): RosteredVsActualData => {
    if (!shiftsWithStaff) return { rows: [], totalRostered: 0, totalActual: 0, hasActual: false }

    const staffMap = new Map<string, { name: string; rostered: number; actual: number }>()

    for (const s of shiftsWithStaff) {
      const sid = s.staff_id
      if (!staffMap.has(sid)) {
        const prof = (s.staff as { org_members: { profiles: { first_name: string; last_name: string } } })
      ...
      entry.rostered += calcHours(s.start_time, s.end_time, s.break_duration_mins || 0)
    }
    ...
    const rows: RosteredVsActualRow[] = Array.from(staffMap.entries())
      ...
      .map(([staffId, v]) => ({
        ...
        variancePct: v.rostered > 0 ? Math.round(((v.actual - v.rostered) / v.rostered) * 100) : 0,
      }))
...
export function useOvertimeReport(venueId: string | undefined, dateRange: DateRange) {
  const from = format(dateRange.from, 'yyyy-MM-dd')
  const to = format(dateRange.to, 'yyyy-MM-dd')

  const { data: shiftsWithStaff, isLoading, error } = useQuery({
    queryKey: ['labourShiftsWithStaff', venueId, from, to],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('roster_shifts')
  ...
  const data = useMemo((): OvertimeData => {
    ...
    for (const s of shiftsWithStaff) {
      ...
      week.hours += calcHours(s.start_time, s.end_time, s.break_duration_mins || 0)
...
export function useUtilizationReport(venueId: string | undefined, dateRange: DateRange) {
  const from = format(dateRange.from, 'yyyy-MM-dd')
  const to = format(dateRange.to, 'yyyy-MM-dd')
  ...
  const { data: staffList, isLoading: staffLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).filter(s =>
        (s.org_members as { is_active: boolean })?.is_active !== false
      )
    },
    enabled: !!venueId,
  })
  ...
  const data = useMemo((): UtilizationData => {
    ...
    const rows: UtilizationRow[] = staffList.map(staff => {
      const prof = (staff.org_members as { profiles: { first_name: string; last_name: string } })?.profiles
      const name = prof ? `${prof.first_name} ${prof.last_name}`.trim() : 'Unknown'
    ...
    const avgUtilization = nonCasual.length > 0
      ? Math.round(nonCasual.reduce((sum, r) => sum + r.utilizationPct, 0) / nonCasual.length)
      : 0
