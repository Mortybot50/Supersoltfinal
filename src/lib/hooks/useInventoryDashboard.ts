...
export function useDashboardMetrics(venueId?: string) {
  ...
  const { data: ingredients, isLoading: ingLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("ingredients")
        .select(
          "id, name, current_stock, par_level, cost_per_unit, category, supplier_id, active",
        )
        .eq("active", true);
  ...
  const metrics = useMemo<DashboardMetrics | null>(() => {
    ...
    return {
      ...
      itemsBelowPar: ingredients.filter((i) => i.current_stock < i.par_level)
        .length,
...
export function useInventoryAlerts(venueId?: string) {
  ...
  const { data: highVariance, isLoading: hvLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: items, error: iErr } = await supabase
        ...
        .select(
          "id, ingredient_name, expected_quantity, actual_quantity, variance",
        )
  ...
  return {
    alerts,
    isLoading: bpLoading || opLoading || hvLoading || npLoading,
  };
}
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const i of ings ?? []) {
        ...
        byCategory[cat] =
          (byCategory[cat] ?? 0) + i.current_stock * i.cost_per_unit;
      }
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      let poQ = supabase
        .from("purchase_orders")
        .select(
          "id, po_number, supplier_name, status, created_at, delivered_at",
        )
        .order("created_at", { ascending: false })
        .limit(10);
    ...
    queryFn: async () => {
      ...
      for (let w = 7; w >= 0; w--) {
        ...
        weeks.push({
          ...
          foodCostPct:
            revenue > 0
              ? Math.round((ingredientCost / revenue) * 1000) / 10
              : null,
          target: 30,
        });
      }
