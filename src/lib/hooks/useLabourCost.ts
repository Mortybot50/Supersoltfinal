...
export function useLabourCost(): LabourCostMetrics {
  ...
  const dailyBreakdown = useMemo((): DayCostBreakdown[] => {
    ...
    return weekDates.map((date, i) => {
      ...
      const dayShifts = shifts.filter((s: RosterShift) => {
        ...
        return format(shiftDate, 'yyyy-MM-dd') === dateStr && s.status !== 'cancelled'
      })

      const uniqueStaff = new Set(dayShifts.map((s: RosterShift) => s.staff_id))

      return {
        date,
        dateStr,
        dayLabel: dayNames[i],
        totalCostCents: dayShifts.reduce((sum: number, s: RosterShift) => sum + s.total_cost, 0),
        baseCostCents: dayShifts.reduce((sum: number, s: RosterShift) => sum + (s.base_cost || s.total_cost), 0),
        penaltyCostCents: dayShifts.reduce((sum: number, s: RosterShift) => sum + (s.penalty_cost || 0), 0),
        totalHours: dayShifts.reduce((sum: number, s: RosterShift) => sum + s.total_hours, 0),
  ...
  const labourPercent = useMemo(() => {
    ...
    return Math.round((weeklyLabourCostCents / avgWeeklyRevenueCents) * 10000) / 100
