...
export function useCOGSMetrics(filters?: COGSFilters): COGSMetricsResult {
  ...
  // Fetch orders for sales total (shared cache)
  ...
  return useMemo(() => {
    ...
    const theoreticalPercent =
      totalSales > 0 ? (theoreticalCOGS / totalSales) * 100 : 0;
    const variance = actualCOGS - theoreticalCOGS;
    const variancePercent =
      theoreticalCOGS !== 0 ? (variance / theoreticalCOGS) * 100 : 0;
    ...
    return {
      metrics: {
        ...
        waste_percent_of_cogs:
          actualCOGS !== 0 ? (wasteValue / actualCOGS) * 100 : 0,
