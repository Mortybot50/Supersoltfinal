...
export const BASE_UNIT_MAP: Record<string, string> = {
  g: 'g', kg: 'g', mL: 'mL', L: 'mL', ea: 'ea', each: 'each',
}
...
export function calculatePackToBaseFactor(unitsPerPack: number, unitSize: number, unit: string): number {
...
export function calculateLineCost(qty: number, recipeUnit: string, costPerBaseUnitCents: number): number {
  const qtyInBase = convertQtyToBaseUnits(qty, recipeUnit)
  return Math.round(qtyInBase * costPerBaseUnitCents)
}

export function calculateCostPerBaseUnit(costCents: number, packToBaseFactor: number): number {
  if (packToBaseFactor === 0) return 0
  return costCents / packToBaseFactor
}

export function formatPackSizeText(unitsPerPack: number, unitSize: number, unit: string): string {
  const sizeStr = unitSize % 1 === 0 ? unitSize.toFixed(0) : unitSize.toString()
  if (unitsPerPack === 1 && unitSize === 1) return '1' + unit
  if (unitsPerPack === 1) return sizeStr + unit
  return unitsPerPack + '×' + sizeStr + unit
}

export function formatBaseUnitCost(costPerBaseUnitCents: number, baseUnit: string): string {
