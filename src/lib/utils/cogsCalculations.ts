import type { Order, OrderItem, Recipe, RecipeItem, WasteLog, PurchaseOrder, StockCount, Ingredient, Supplier, MenuItem } from '@/types'
import type { CategoryCOGS, SupplierCOGS, RecipeCOGS, WasteImpact, COGSAnomaly, IngredientUsage } from '@/types/cogs.types'
...
export function calculateCOGSVariance(
  ...
  const variancePercent = theoreticalCOGS !== 0 
    ? (varianceValue / theoreticalCOGS) * 100 
    : null
 ...
 */
export function calculateOpeningStockValue(
...
export function calculateCategoryBreakdown(
  ...
  const totalActualCOGS = Array.from(categoryMap.values()).reduce((sum, cat) => sum + cat.actualCost, 0)
  ...
  categoryMap.forEach((data, category) => {
    ...
    categories.push({
      ...
      variance,
      variance_percent: data.theoreticalCost !== 0 
        ? (variance / data.theoreticalCost) * 100 
        : 0,
      share_of_total_cogs: totalActualCOGS !== 0 ? (data.actualCost / totalActualCOGS) * 100 : 0,
...
export function calculateSupplierPerformance(
  ...
  const totalPurchases = Array.from(supplierStats.values())
    .reduce((sum, s) => sum + s.totalPurchases, 0)
  ...
  return suppliers.map(supplier => {
    ...
    const avgDeliveryTime = stats.avgDaysToDeliver.length > 0
      ? stats.avgDaysToDeliver.reduce((a, b) => a + b, 0) / stats.avgDaysToDeliver.length
      : 0
    
    const reliabilityScore = stats.orderCount > 0
      ? (stats.onTimeCount / stats.orderCount) * 100
      : 0
    ...
    return {
      ...
      share_of_total_purchases: totalPurchases !== 0 
...
export function calculateWasteImpact(
  ...
  const categoryWaste = new Map<string, {
    value: number
    qty: number
  ...
  // Top wasted items
  const itemWaste = new Map<string, {
    value: number
    qty: number
    unit: string
    reasons: string[]
  ...
  return {
    ...
    waste_as_percent_of_cogs: totalCOGS !== 0 ? (totalWaste / totalCOGS) * 100 : 0,
    waste_by_category: Array.from(categoryWaste.entries()).map(([category, data]) => {
...
export function calculateRecipeCOGS(
  ...
  const recipeSales = new Map<string, {
    timesSold: number
    avgPrice: number
  }>()
  ...
  return recipes.map(recipe => {
    ...
    const gpPercent = sales.avgPrice !== 0
