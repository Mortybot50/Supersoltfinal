...
export function calculateShiftCostBreakdown(
  ...
  if (minHours > 0 && roundedHours < minHours) {
    warnings.push(`Shift is ${roundedHours.toFixed(1)}h — below ${minHours}h minimum engagement for ${employmentType}`)
  }

  // Break rule checks
  if (roundedHours > AU_HOSPITALITY_PENALTY_RATES.meal_break_threshold_hours && breakMinutes < AU_HOSPITALITY_PENALTY_RATES.meal_break_duration_minutes) {
    warnings.push(`Shift >5h requires 30-min unpaid meal break (only ${breakMinutes}min scheduled)`)
  }
  if (roundedHours > AU_HOSPITALITY_PENALTY_RATES.rest_break_threshold_hours && breakMinutes < AU_HOSPITALITY_PENALTY_RATES.rest_break_duration_minutes) {
    warnings.push(`Shift >4h requires 10-min paid rest break`)
  }

  // Determine penalty
  const { penaltyType, penaltyMultiplier } = calculatePenaltyRate(date, startTime, endTime, state, isCasual)
  ...
  if (employmentType === 'full-time') {
    ...
    if (roundedHours > 10) {
      ...
      warnings.push(`${overtimeHours.toFixed(1)}h overtime (shift exceeds 10h/day)`)
    }
    // Weekly overtime: >38h
    ...
    if (overtimeHours > 0) {
      ...
      if (overtimeHours <= 2) {
        overtimeMultiplier = AU_HOSPITALITY_PENALTY_RATES.overtime_first_2_hours
      } else {
        overtimeMultiplier = AU_HOSPITALITY_PENALTY_RATES.overtime_after_2_hours
      }
    }
  }
  ...
  const penaltyCostCents = Math.round(ordinaryHours * hourlyRateCents * (penaltyMultiplier - 1))
  const overtimeCostCents = overtimeHours > 0
    ? Math.round(overtimeHours * hourlyRateCents * overtimeMultiplier) - Math.round(overtimeHours * hourlyRateCents)
    : 0

  const totalPenaltyCents = penaltyCostCents + overtimeCostCents
  const totalCostCents = Math.round(roundedHours * hourlyRateCents * penaltyMultiplier)
    + (overtimeHours > 0 ? Math.round(overtimeHours * hourlyRateCents * (overtimeMultiplier - penaltyMultiplier)) : 0)
...
export function calculateBudgetVariance(
  ...
  if (!budget) {
    return { budgeted: 0, actual: actualCost, variance: 0, variancePercent: 0, status: 'under' }
  }
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      ...
      if (gapHours >= 0 && gapHours < 10) {
        warnings.push({
          ...
          details: { gap_hours: gapHours, shift_date: new Date(next.date).toISOString().split('T')[0] },
          acknowledged: false,
        })
      }
    }
...
export function detectMinimumEngagementWarnings(
  ...
  activeShifts.forEach((shift) => {
    ...
    if ((empType === 'casual' || empType === 'part-time') && shift.total_hours < 3) {
      warnings.push({
...
export function detectOvertimeWarnings(
  ...
  activeShifts.forEach((s) => {
    if (!byStaff[s.staff_id]) byStaff[s.staff_id] = { name: s.staff_name, hours: 0 }
  ...
  for (const [staffId, data] of Object.entries(byStaff)) {
    if (data.hours > 38) {
      warnings.push({
        staffId, staffName: data.name, hours: data.hours,
  ...
  activeShifts.forEach((s) => {
    ...
    if (!byStaffDay[key]) byStaffDay[key] = { name: s.staff_name, hours: 0, date: dateStr }
  ...
  for (const [key, data] of Object.entries(byStaffDay)) {
    if (data.hours > 10) {
      warnings.push({
        staffId: key.split('_')[0], staffName: data.name, hours: data.hours,
...
export function getAllRosterWarnings(
  ...
  warnings.push(...detectMinimumEngagementWarnings(shifts, staffEmploymentTypes))
...
export function getShiftsForWeek(shifts: RosterShift[], weekStart: Date): RosterShift[] {
...
const ROLE_COLORS: Record<string, { bg: string; text: string; border: string; light: string }> = {
  kitchen: { bg: 'bg-blue-500', text: 'text-blue-700', border: 'border-blue-300', light: 'bg-blue-50 dark:bg-blue-950/30' },
  foh: { bg: 'bg-green-500', text: 'text-green-700', border: 'border-green-300', light: 'bg-green-50 dark:bg-green-950/30' },
  bar: { bg: 'bg-purple-500', text: 'text-purple-700', border: 'border-purple-300', light: 'bg-purple-50 dark:bg-purple-950/30' },
  manager: { bg: 'bg-gray-500', text: 'text-gray-700', border: 'border-gray-300', light: 'bg-gray-50 dark:bg-gray-800/30' },
  management: { bg: 'bg-gray-500', text: 'text-gray-700', border: 'border-gray-300', light: 'bg-gray-50 dark:bg-gray-800/30' },
  supervisor: { bg: 'bg-teal-500', text: 'text-teal-700', border: 'border-teal-300', light: 'bg-teal-50 dark:bg-teal-950/30' },
  crew: { bg: 'bg-amber-500', text: 'text-amber-700', border: 'border-amber-300', light: 'bg-amber-50 dark:bg-amber-950/30' },
}

const DEFAULT_ROLE_COLOR = { bg: 'bg-slate-500', text: 'text-slate-700', border: 'border-slate-300', light: 'bg-slate-50 dark:bg-slate-800/30' }

export function getRoleColor(role: string): { bg: string; text: string; border: string; light: string } {
...
export function applyShiftTemplate(
  template: { start_time: string; end_time: string; break_minutes: number; role: string },
  staffId: string,
  staffName: string,
  date: Date,
  hourlyRateCents: number,
