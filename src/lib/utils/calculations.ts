 ...
 */
export function calculateRefundMetrics(orders: Types.Order[]): Types.RefundMetrics {
  ...
  const totalRefundValue = Math.abs(refunds.reduce((sum, o) => sum + o.net_amount, 0))
  
  return {
    refund_count: refunds.length,
    refund_rate_percent: totalOrders > 0 ? (refunds.length / totalOrders) * 100 : 0,
    refund_value: totalRefundValue,
    refund_value_percent: totalSales > 0 ? (totalRefundValue / totalSales) * 100 : 0,
 ...
 */
export function calculateChannelMix(orders: Types.Order[]): Types.ChannelMetrics[] {
  const validOrders = orders.filter(o => !o.is_void)
  const totalSales = calculateNetSales(validOrders)
 ...
 */
export function calculatePaymentMix(tenders: Types.Tender[]): Types.PaymentMix[] {
...
export function calculateTotalLabourHours(timesheets: Types.Timesheet[]): number {
  return timesheets
    .filter(t => t.status === 'approved')
    .reduce((sum, t) => sum + t.total_hours, 0)
}

export function calculateTotalLabourCost(timesheets: Types.Timesheet[]): number {
  return timesheets
    .filter(t => t.status === 'approved')
...
export function calculateTotalStockValue(ingredients: Types.Ingredient[]): number {
  return ingredients
    .filter(ing => ing.active)
    .reduce((sum, ing) => sum + (ing.current_stock * ing.cost_per_unit), 0)
}

export function calculateItemsBelowPar(ingredients: Types.Ingredient[]): number {
  return ingredients.filter(ing => ing.active && ing.current_stock < ing.par_level).length
}
...
export function calculateTotalWasteValue(wasteLogs: Types.WasteEntry[]): number {
  return wasteLogs.reduce((sum, log) => sum + log.value, 0)
}

export function calculateStockVariance(stockCounts: Types.StockCountItem[]): number {
...
export function calculateUpcomingObligations(
  ...
  return obligations
    .filter(o => o.status === 'upcoming' && new Date(o.due_date) <= futureDate)
