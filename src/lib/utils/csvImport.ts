...
export function parseStaffCSV(csvText: string): CSVImportResult<StaffCSVRow> {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const firstName =
      get("first_name") || get("firstname") || get("name")?.split(" ")[0] || "";
    const lastName =
      get("last_name") ||
      get("lastname") ||
      get("name")?.split(" ").slice(1).join(" ") ||
      "";
    ...
    if (!rate || isNaN(rate)) {
      errors.push({
        row: i + 1,
        message: `Invalid hourly rate: "${rateStr}" for ${firstName} ${lastName}`,
      });
      continue;
    }
    ...
    data.push({
      ...
      employment_type: ["casual", "part_time", "full_time"].includes(empType)
        ? empType
        : "casual",
      base_hourly_rate: rate,
      award_classification:
        get("award_classification") ||
        get("award_level") ||
        get("award") ||
        undefined,
...
export function parseIngredientCSV(
  csvText: string,
): CSVImportResult<IngredientCSVRow> {
  const rows = parseCSV(csvText);
  if (rows.length < 2)
    return {
      data: [],
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const packCostStr =
      get("pack_cost") || get("cost") || get("price") || get("unit_cost");
    const packCost = parseFloat(packCostStr);
    if (!packCost || isNaN(packCost)) {
      warnings.push({
        row: i + 1,
        message: `No cost for ${name} — will need to be entered later`,
      });
    }
    ...
    data.push({
      ...
      reorder_point:
        parseFloat(
          get("reorder_point") || get("reorder") || get("min_stock"),
        ) || undefined,
