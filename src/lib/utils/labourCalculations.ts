...
import type { RoleLabourBreakdown, OvertimeAnalysis, RosterCompliance, LabourEfficiency } from '@/types/labour.types'
 ...
 */
export function calculateRosteredHours(
  shifts: RosterShift[]
): number {
...
export function calculateRoleBreakdown(
  ...
  const totalCost = Array.from(roleMap.values())
  ...
  const breakdown = Array.from(roleMap.entries()).map(([role, data]) => ({
    ...
    overtime_percent: data.hours !== 0 ? (data.overtimeHours / data.hours) * 100 : 0
  }))
...
export function calculateOvertimeAnalysis(
  ...
  const totalOvertimeHours = Array.from(overtimeMap.values())
    .reduce((sum, d) => sum + d.hours, 0)
  const totalOvertimeCost = Array.from(overtimeMap.values())
    .reduce((sum, d) => sum + d.cost, 0)
  
  const totalLabourCost = calculateTotalLabourCost(timesheets)
  
  const staffWithOvertime = Array.from(overtimeMap.entries()).map(([staffId, data]) => {
  ...
  return {
    ...
    overtime_as_percent_of_total: totalLabourCost !== 0 
      ? (totalOvertimeCost / totalLabourCost) * 100 
      : 0,
  ...
  timesheets.forEach(ts => {
    ...
    if (ts.total_hours > 12) {
      issues.push({
        ...
        suggested_action: 'Review shift scheduling and approve overtime if necessary'
      })
    }
    ...
    if (ts.total_hours >= 5 && ts.break_minutes < 30) {
      issues.push({
        ...
        suggested_action: 'Ensure 30 minute unpaid break for shifts over 5 hours'
      })
    }
  })
...
export function calculateLabourEfficiency(
  ...
  const salesPerLabourHour = calculateSalesPerLabourHour(totalSales, totalHours)
  const salesPerLabourDollar = calculateSalesPerLabourDollar(totalSales, totalCost)
  
  const benchmarkSalesPerLH = 15000
  
  const variance = salesPerLabourHour !== 0 
  ...
  return {
    ...
    transactions_per_labour_hour: totalHours !== 0 ? totalTransactions / totalHours : 0,
