...
function mapDBRecipeIngredientToApp(
  ...
  const subRecipeId = (db as Record<string, unknown>).sub_recipe_id as
    | string
    | undefined;
  return {
    id: db.id,
...
export async function loadRecipesFromDB(): Promise<{
  ...
  for (const id of subRecipeIds) {
    ...
    if (r) {
      subRecipeMap[id] = {
        name: r.name,
        cost_per_serve: r.cost_per_serve ?? 0,
      };
    }
  }
  ...
  const recipeIngredients = (riData || []).map((ri) => {
    ...
    return mapDBRecipeIngredientToApp(
      ri,
      ingredientsMap[ri.ingredient_id ?? ""],
    );
  });
...
export async function saveRecipeToDB(
  ...
  const validIngredients = ingredients.filter(
    (ing) => ing.product_id || ing.is_sub_recipe,
  );
  if (validIngredients.length > 0) {
    const dbIngredients = validIngredients.map((ing, index) => ({
      ...
      sub_recipe_id: ing.is_sub_recipe
        ? (ing.sub_recipe_id ?? ing.product_id)
        : null,
