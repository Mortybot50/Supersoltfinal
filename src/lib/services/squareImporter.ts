...
class SquareImporter {
  ...
  private async importCatalog(orgId: string, accessToken: string): Promise<{ count: number }> {
    ...
    for (const item of menuItems) {
      await supabase.from('menu_items').insert({
        ...
        price: Number(item.item_data.variations?.[0]?.item_variation_data?.price_money?.amount || 0) / 100,
  ...
  private async importTeam(orgId: string, accessToken: string): Promise<{ count: number }> {
  ...
  private async importSalesHistory(orgId: string, accessToken: string): Promise<{ count: number }> {
  ...
  private async importVenueDetails(orgId: string, accessToken: string): Promise<void> {
