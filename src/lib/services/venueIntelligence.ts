...
class VenueIntelligence {
  detectVenueType(menuItems: any[], businessHours?: any): VenueType {
    ...
    const coffeeItems = itemNames.filter(
      (name) =>
        name.includes("latte") ||
    ...
    const alcoholItems = itemNames.filter(
      (name) =>
        name.includes("beer") ||
        name.includes("wine") ||
    ...
    const qsrIndicators = itemNames.filter(
      (name) =>
        name.includes("combo") ||
        name.includes("meal deal") ||
        name.includes("value") ||
        name.includes("kids meal"),
    ).length;
