...
export async function getDemandForecasts(
  ...
  if (error)
    throw new Error(`Failed to fetch demand forecasts: ${error.message}`);
...
> {
  ...
  return Array.from(byItem.entries()).map(
    ([menuItemId, { menuItemName, mapes, lastForecastAt }]) => ({
      menuItemId,
      menuItemName,
      mape:
        Math.round((mapes.reduce((s, v) => s + v, 0) / mapes.length) * 100) /
        100,
