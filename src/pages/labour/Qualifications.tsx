...
import {
  Plus, MoreVertical, CheckCircle, AlertTriangle, XCircle, MinusCircle, Shield,
} from 'lucide-react'
...
type MatrixStatus = 'valid' | 'expiring' | 'expired' | 'not_assigned' | 'not_required'
...
function MatrixCell({ status }: { status: MatrixStatus }) {
  if (status === 'valid')        return <div className="flex justify-center"><CheckCircle className="h-5 w-5 text-green-600" /></div>
  if (status === 'expiring')     return <div className="flex justify-center"><AlertTriangle className="h-5 w-5 text-amber-500" /></div>
  if (status === 'expired')      return <div className="flex justify-center"><XCircle className="h-5 w-5 text-destructive" /></div>
  if (status === 'not_assigned') return <div className="flex justify-center"><MinusCircle className="h-5 w-5 text-muted-foreground/30" /></div>
...
function QualTypeDialog({ open, onOpenChange, orgId, editing, onSaved, onDeleted }: QualTypeDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setValidity(editing?.validity_months ? String(editing.validity_months) : '')
  ...
  const handleSave = async () => {
    ...
    let error = null
    if (editing) {
      const res = await supabase.from('qualification_types').update(payload).eq('id', editing.id).select().single()
      data = res.data as QualificationType; error = res.error
    } else {
      const res = await supabase.from('qualification_types').insert(payload).select().single()
      data = res.data as QualificationType; error = res.error
    }
    setSaving(false)
  ...
  const handleDelete = async () => {
    if (!editing) return
    const { error } = await supabase.from('qualification_types').delete().eq('id', editing.id)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{editing ? 'Edit' : 'New'} Qualification Type</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            ...
            <datalist id="hospo-presets">
              {HOSPO_QUAL_PRESETS.map(p => <option key={p} value={p} />)}
            </datalist>
          </div>
          <div>
...
function AssignDialog({ open, onOpenChange, orgId, qualTypes, staffList, existingQuals, onSaved }: AssignDialogProps) {
  ...
  const takenTypeIds = existingQuals.filter(q => q.staff_id === staffId).map(q => q.qualification_type_id)

  useEffect(() => {
    if (issueDate && selectedType?.validity_months) {
      setExpiry(addMonths(new Date(issueDate), selectedType.validity_months).toISOString().split('T')[0])
    }
  }, [issueDate, selectedType])

  const handleSave = async () => {
    if (!staffId || !typeId) { toast.error('Select staff and qualification type'); return }
    setSaving(true)
    const { data, error } = await supabase.from('staff_qualifications').insert({
      org_id: orgId,
      staff_id: staffId,
    ...
    onOpenChange(false)
    setStaffId(''); setTypeId(''); setIssue(''); setExpiry(''); setCertNum('')
  }

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>Assign Qualification</DialogTitle></DialogHeader>
        <div className="space-y-4">
          <div>
            <Label>Staff Member *</Label>
            <Select value={staffId} onValueChange={v => { setStaffId(v); setTypeId('') }}>
          ...
          <div>
            ...
            <Select value={typeId} onValueChange={setTypeId} disabled={!staffId}>
              <SelectTrigger><SelectValue placeholder="Select type…" /></SelectTrigger>
              <SelectContent>
                {qualTypes.filter(t => !takenTypeIds.includes(t.id)).map(t => (
          ...
          <div>
            ...
            <Input value={certNum} onChange={e => setCertNum(e.target.value)} placeholder="Optional" />
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
...
export default function Qualifications() {
  ...
  const [editingType, setEditingType]           = useState<QualificationType | undefined>()
  ...
  const loadData = useCallback(async () => {
    ...
    const [{ data: types }, { data: quals }] = await Promise.all([
      supabase.from('qualification_types').select('*').eq('org_id', currentOrg.id).order('name'),
      supabase.from('staff_qualifications').select('*').eq('org_id', currentOrg.id),
    ])
  ...
  const handleQualAssigned = (q: StaffQualification) => {
    setStaffQuals(prev => [...prev, { ...q, status: computeStatus(q.expiry_date) }])
  }
        ...
        <div className="flex gap-2">
          {(expiredCount > 0 || expiringCount > 0) && (
            <div className="flex items-center gap-1 text-sm">
              ...
              {expiringCount > 0 && <Badge className="bg-amber-100 text-amber-800 border-amber-200">{expiringCount} expiring</Badge>}
            </div>
          )}
          <Button variant="outline" size="sm" onClick={() => { setEditingType(undefined); setTypeDialogOpen(true) }}>
        ...
        <Tabs defaultValue="types">
          <TabsList className="mb-4">
            ...
            <TabsTrigger value="staff">
              ...
              {alerts.length > 0 && <AlertTriangle className="h-3 w-3 ml-1 text-amber-500" />}
            </TabsTrigger>
            ...
            {qualTypes.length === 0 ? (
              <Card className="p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Add common types like RSA, Food Safety, First Aid, WWC, Barista.
                </p>
                <Button onClick={() => { setEditingType(undefined); setTypeDialogOpen(true) }}>
              ...
              <Table>
                ...
                <TableBody>
                  {qualTypes.map(t => {
                    const holdingCount = staffQuals.filter(q => q.qualification_type_id === t.id).length
                    return (
                      <TableRow key={t.id}>
                        <TableCell className="font-medium">{t.name}</TableCell>
                        <TableCell className="text-sm text-muted-foreground">{t.description || '—'}</TableCell>
                        <TableCell className="text-sm">
                          {t.validity_months ? `${t.validity_months} months` : 'No expiry'}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-sm">{holdingCount}</TableCell>
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => { setEditingType(t); setTypeDialogOpen(true) }}>
                                Edit
                              </DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
          ...
          <TabsContent value="staff">
            {staffQuals.length === 0 ? (
              <Card className="p-8 text-center">
                <p className="font-medium mb-2">No staff qualifications assigned yet</p>
                <p className="text-sm text-muted-foreground mb-4">
                  Assign qualifications to staff members to track expiry and compliance.
                </p>
              ...
              <Table>
                ...
                <TableBody>
                  {staffQuals.map(q => {
                    const staffMember = staffList.find(s => s.id === q.staff_id)
                    const qualType    = qualTypes.find(t => t.id === q.qualification_type_id)
                    return (
                      <TableRow key={q.id}>
            ...
            {qualTypes.length === 0 || activeStaff.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <div className="mb-3 flex items-center gap-4 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><CheckCircle className="h-4 w-4 text-green-600" /> Valid</span>
                  <span className="flex items-center gap-1"><AlertTriangle className="h-4 w-4 text-amber-500" /> Expiring (&lt;30 days)</span>
                  <span className="flex items-center gap-1"><XCircle className="h-4 w-4 text-destructive" /> Expired</span>
                  <span className="flex items-center gap-1"><MinusCircle className="h-4 w-4 text-muted-foreground/30" /> Not assigned</span>
                </div>
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="min-w-[140px]">Staff Member</TableHead>
                      {qualTypes.map(t => (
                        <TableHead key={t.id} className="text-center min-w-[110px]">
                      ...
                      <TableRow key={s.id}>
                        ...
                        {qualTypes.map(t => {
                          const qual = staffQuals.find(
                            q => q.staff_id === s.id && q.qualification_type_id === t.id
                          )
                          const isRequired = t.required_for_roles.includes(s.role)
