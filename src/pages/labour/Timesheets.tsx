...
import { Clock, DollarSign, CheckCircle, XCircle, MoreVertical, Check, X, FileCheck, Calendar, ListPlus, Edit3, Loader2 } from "lucide-react"
import { useDataStore } from "@/lib/store/dataStore"
import { useRosterMetrics } from "@/lib/hooks/useRosterMetrics"
import { Timesheet, RosterShift } from "@/types"
import { format, startOfWeek, endOfWeek, addDays, isWithinInterval, eachDayOfInterval } from "date-fns"
...
export default function Timesheets() {
  ...
  const generatableCount = useMemo(() => {
    ...
    return rosterShifts.filter((s) => {
      if (s.status === "cancelled" || s.is_open_shift) return false
      const shiftDate = new Date(s.date)
      if (!isWithinInterval(shiftDate, { start: weekStart, end: weekEnd })) return false
                      ...
                      {ts.status === "pending" ? (
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => handleAdjust(ts)}>
      ...
      <Dialog open={adjustDialogOpen} onOpenChange={setAdjustDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              {adjustTarget && `${adjustTarget.staff_name} — ${format(new Date(adjustTarget.date), "EEE, MMM d")}`}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveAdjustment} disabled={!adjustReason.trim()}>Save Adjustment</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PageShell>
