...
const DEFAULT_QUAL_TYPES = [
  {
    ...
    validity_months: 36 as number | null,
    required_for_roles: ["manager", "supervisor", "crew"],
  },
  {
    name: "Food Safety Certificate",
    description: "FSANZ Food Safety Handler Certificate",
    validity_months: 60 as number | null,
    required_for_roles: ["manager", "supervisor", "crew"],
  },
  {
    name: "First Aid Certificate",
    description: "Provide First Aid (HLTAID011)",
    validity_months: 36 as number | null,
    required_for_roles: ["manager", "supervisor"],
  },
  {
    name: "Working With Children Check",
    description: "WWCC — state-specific",
    validity_months: 60 as number | null,
    required_for_roles: ["manager", "supervisor", "crew"],
  },
  {
    name: "Barista Certificate",
    description: "Espresso machine operation and coffee skills",
    validity_months: null,
    required_for_roles: ["crew"],
  },
  {
    name: "RSG",
    description: "Responsible Service of Gambling",
    validity_months: 36 as number | null,
    required_for_roles: ["manager", "supervisor"],
  },
  {
    name: "Liquor License",
    description: "Personal liquor licence or approval",
    validity_months: 12 as number | null,
    required_for_roles: ["manager"],
  },
  {
    name: "Manual Handling Certificate",
    description: "Safe manual handling and lifting techniques",
    validity_months: 24 as number | null,
    required_for_roles: ["manager", "supervisor", "crew"],
  },
];
...
function AssignQualDialog({
  ...
  const availableTypes = qualTypes.filter(
    (t) => !existing.some((e) => e.qualification_type_id === t.id),
  );
  ...
  const handleSave = async () => {
    if (!typeId) {
      toast.error("Select a qualification type");
      return;
    }
    setSaving(true);
...
export default function StaffDetailPage() {
  ...
  const { staff, onboardingSteps, onboardingDocuments, updateStaff } =
    useDataStore();
  ...
  const loadQuals = useCallback(async () => {
    ...
    const [{ data: types }, { data: quals }] = await Promise.all([
      ...
      supabase
        .from("staff_qualifications")
        .select("*, qualification_types(name, validity_months)")
        .eq("staff_id", id),
    ]);

    // Seed default AU hospitality qual types if org has none
    if (types !== null && types.length === 0) {
      const seeds = DEFAULT_QUAL_TYPES.map((t) => ({
        ...t,
  ...
  const handleQualAdded = (q: StaffQualification) => {
    setStaffQuals((prev) => [
      ...prev,
      { ...q, status: computeQualStatus(q.expiry_date) },
    ]);
  };
  ...
  const [editingSection, setEditingSection] = useState<
    "personal" | "address" | "emergency" | "pay" | null
  >(null);
  ...
  const emergencyForm = useForm<z.infer<typeof emergencySchema>>({
  ...
  useEffect(() => {
    if (member)
      setNotes(
        (member as Staff & { manager_notes?: string }).manager_notes ?? "",
      );
  }, [member]);
    ...
    return (
      <div className="p-6">
        <Card className="p-6 text-center space-y-4">
          ...
          <Button onClick={() => navigate("/workforce/people")}>
            Back to People
          </Button>
        </Card>
      </div>
      ...
      <div className="space-y-4">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            {!isEditingAddress && (
              <Button variant="outline" size="sm" onClick={startEditAddress}>
                <Pencil className="h-3 w-3 mr-1" /> Edit
              </Button>
            )}
          </div>
          {isEditingAddress ? (
            <Form {...addressForm}>
              <form
                onSubmit={addressForm.handleSubmit((v) => saveSection(v))}
                className="space-y-3"
              >
                <div className="grid grid-cols-2 gap-3">
                  <FormField
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            {!isEditingEmergency && (
              <Button variant="outline" size="sm" onClick={startEditEmergency}>
                <Pencil className="h-3 w-3 mr-1" /> Edit
              </Button>
            )}
          </div>
          {isEditingEmergency ? (
            <Form {...emergencyForm}>
              <form
                onSubmit={emergencyForm.handleSubmit((v) => saveSection(v))}
                className="space-y-3"
              >
                <div className="grid grid-cols-2 gap-3">
                  <FormField
  ...
  const PayTab = () => {
    ...
    const startEditPay = () => {
      payForm.reset({
        hourly_rate_display:
          member.hourly_rate > 0 ? (member.hourly_rate / 100).toFixed(2) : "",
        employment_type: member.employment_type as
          | "full-time"
          | "part-time"
          | "casual"
          | undefined,
    ...
    return (
      <div className="space-y-4">
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold flex items-center gap-2">
              <DollarSign className="h-4 w-4" /> Pay Conditions
            </h3>
            {!isEditingPay && (
              <Button variant="outline" size="sm" onClick={startEditPay}>
                <Pencil className="h-3 w-3 mr-1" /> Edit
              </Button>
            )}
          </div>
            ...
            <Form {...payForm}>
              <form
                onSubmit={payForm.handleSubmit((v) =>
                  saveSection({
                    hourly_rate: v.hourly_rate_display
                      ? Math.round(parseFloat(v.hourly_rate_display) * 100)
                      : member.hourly_rate,
                    ...
                    render={({ field }) => (
                      <FormItem className="col-span-2">
                        ...
                        <FormControl>
                          <Input
                            {...field}
                            placeholder="e.g. Level 2 — Hospitality Industry Award"
                          />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <InfoRow
              label="Using Employer Default"
              value={member.super_use_employer_default ? "Yes" : "No"}
            />
          </div>
        </Card>
      </div>
    ...
    <div className="space-y-4">
      <Card className="p-5">
        <h3 className="font-semibold mb-4 flex items-center gap-2">
          <Clock className="h-4 w-4" /> Time & Attendance Settings
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
      ...
      <Card className="p-5">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          View and manage this staff member&apos;s timesheet entries on the
          Timesheets page.
        </p>
      </Card>
    </div>
    ...
    <div className="space-y-4">
      <Card className="p-5">
        ...
        <p className="text-xs text-muted-foreground mt-3">
          Leave balances are calculated from your payroll system. Connect a
          payroll integration to display live data.
        </p>
      </Card>
        ...
        <div className="grid grid-cols-7 gap-1">
          {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
            <div key={day} className="text-center">
              ...
              <div className="h-8 rounded bg-muted flex items-center justify-center text-xs text-muted-foreground">
                —
              </div>
            </div>
          ))}
  ...
  const HRTab = () => {
    const expiredQuals = staffQuals.filter(
      (q) => q.status === "expired",
    ).length;
    const expiringQuals = staffQuals.filter(
      (q) => q.status === "expiring",
    ).length;
      ...
      <div className="space-y-4">
        ...
        <Card className="p-5">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <Badge
              variant={
                member.onboarding_status === "roster_ready"
                  ? "default"
                  : "outline"
              }
                  ...
                  <TableRow key={q.id}>
                    <TableCell className="font-medium">
                      {q.qualification_types?.name ??
                        qualTypes.find((t) => t.id === q.qualification_type_id)
                          ?.name ??
                        "—"}
                    </TableCell>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button
                  size="sm"
                  onClick={async () => {
                    const success = await updateStaffInDB(member.id, {
                      manager_notes: notes,
                    } as Partial<Staff>);
                    if (success) {
        ...
        <BreadcrumbList>
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link to="/workforce/people">People</Link>
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
