...
function leaveDuration(req: LeaveRequest): number {
  const ms =
    parseISO(req.end_date).getTime() - parseISO(req.start_date).getTime();
...
function LeaveDetailSheet({
  ...
  useEffect(() => {
    ...
    getLeaveBalances(request.staff_id, new Date(staff.start_date)).then(
      setBalances,
    );
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {leaveDuration(request)} day
                    {leaveDuration(request) !== 1 ? "s" : ""}
                  </p>
                </div>
                ...
                <div className="col-span-2">
                  ...
                  <p className="text-sm">
                    {format(parseISO(request.created_at), "d MMM yyyy")}
                  </p>
                </div>
              </div>
            </TabsContent>
                ...
                <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200">
                  ...
                  <div className="text-sm">
                    <p className="font-medium text-amber-800">
                      {impact.shiftsAffected} shift
                      {impact.shiftsAffected !== 1 ? "s" : ""} affected
                    </p>
                    <p className="text-amber-700 mt-0.5">
                      {impact.hoursAffected.toFixed(1)} hours will need to be
                      covered.
                    </p>
                  </div>
                </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-sm font-semibold">
                          {b.remaining_days} day
                          {b.remaining_days !== 1 ? "s" : ""} remaining
                        </span>
                      </div>
        ...
        <thead>
          <tr>
            ...
            {DAYS.map((d) => (
              <th
                key={d}
                className="text-center py-2 px-2 font-medium text-muted-foreground min-w-[72px]"
              >
                {d}
              </th>
            ))}
...
function LeaveCalendar({
  ...
  function getLeavesForDay(
    date: Date,
  ): Array<{ request: LeaveRequest; staffName: string }> {
    return approvedLeave
      ...
      .map((r) => ({
        request: r,
        staffName: staffMap[r.staff_id]?.name ?? "Unknown",
      }));
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-px bg-border rounded-lg overflow-hidden border">
        {dayHeaders.map((d) => (
          <div
            key={d}
            className="bg-muted/50 text-xs font-medium text-muted-foreground text-center py-1.5"
          >
            {d}
          </div>
        ))}
        {paddedDays.map((date, i) => {
          if (!date) {
            return (
              <div key={`pad-${i}`} className="bg-background min-h-[60px]" />
            );
          }
...
export default function AvailabilityLeave() {
  ...
  const { data: staff = [] } = useQuery<StaffRow[]>({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("staff")
        .select(
          "id, start_date, org_members!inner(profiles!inner(first_name, last_name))",
        )
        .eq("venue_id", venueId)
        .order("created_at");
  ...
  const declinedLeave = allLeave.filter(
