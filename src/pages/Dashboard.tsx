...
export default function Dashboard() {
  ...
  const gpPct =
    netRevenue > 0 ? ((netRevenue - actualCOGS) / netRevenue) * 100 : 0;
      ...
      return sales.orders
        .filter(
          (o) =>
            !o.is_void &&
            !o.is_refund &&
            isSameDay(new Date(o.order_datetime), day),
        )
        ...
        ? (prevSales.orders || [])
            .filter(
              (o) =>
                !o.is_void &&
                !o.is_refund &&
                isSameDay(new Date(o.order_datetime), prevDay),
            )
            .reduce((sum, o) => sum + o.net_amount / 100, 0)
        : 0;
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {posStatus?.connected ? (
            ...
            <Button
              ...
              size="sm"
              variant="outline"
              className="h-7 text-xs hidden md:flex gap-1.5 border-border/60"
            >
            ...
            <div>
              ...
              {isLoading ? (
                ...
                <p className="text-4xl font-bold tracking-tight tabular-nums">
                  $
                  {netRevenue.toLocaleString("en-AU", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
                </p>
          ...
          <div className="lg:col-span-2 rounded-xl border border-border/60 bg-card shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-border/40 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-0.5 rounded bg-slate-300 dark:bg-slate-600" />{" "}
                  Previous
                </span>
              </div>
            </div>
              ...
              {rosterToday.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-[200px] overflow-auto">
                  {rosterToday.map((shift) => (
                    <div
                      key={shift.id}
                      className="flex items-center justify-between text-sm px-3 py-2 rounded-lg bg-muted/40 hover:bg-muted/60 transition-colors"
                    >
                      <div>
                        <div className="font-medium text-sm">
                          {shift.staff_name || "Open"}
                        </div>
                  ...
                  <div className="pt-2 border-t border-border/40 text-xs text-muted-foreground flex justify-between tabular-nums">
                    <span>
                      ...
                      {rosterToday
                        .reduce((s, sh) => s + (sh.total_hours ?? 0), 0)
                        .toFixed(1)}
                      h
                    </span>
              ...
              {alerts.length === 0 ? (
                ...
                <div className="space-y-2 max-h-[220px] overflow-auto">
                  {alerts.map((alert, i) => (
                    ...
                    >
                      <AlertTriangle
                        className={`h-4 w-4 shrink-0 mt-0.5 ${alert.type === "warning" ? "text-amber-500" : "text-blue-500"}`}
                      />
              ...
              {sales.paymentMix.map((pm) => (
                ...
                >
                  ...
                  <span className="font-medium tabular-nums">
                    {fmtDollars(pm.amount)}
                  </span>
                  <span className="text-xs text-muted-foreground tabular-nums">
                    ({pm.share_pct.toFixed(0)}%)
                  </span>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card p-5 shadow-sm card-hover">
      <div className="flex items-start justify-between mb-3">
        {Icon && (
          <div
            className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${iconBg || "bg-brand-50 dark:bg-brand-900/20"}`}
          >
            <Icon
              className={`h-5 w-5 ${iconColor || "text-brand-600 dark:text-brand-400"}`}
            />
          </div>
        )}
      ...
      <div className="flex items-center gap-2 mt-2">
        {change !== undefined && change !== 0 ? (
          <span
            className={`inline-flex items-center gap-0.5 text-xs font-medium ${change >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400"}`}
          >
            {change >= 0 ? (
          ...
          <span
            className={`inline-flex items-center gap-1 text-xs font-medium ${statusColors[status]}`}
          >
            <span
