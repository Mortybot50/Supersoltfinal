...
export default function Ingredients() {
  ...
  const handleSave = async () => {
    ...
    const packToBaseFactor = calculatePackToBaseFactor(
      unitsPerPack,
      unitSize,
      formData.unit,
    );
  ...
  const lowStockCount = useMemo(() => {
    return ingredients.filter(
      (i) => i.current_stock <= i.reorder_point && i.active,
    ).length;
  }, [ingredients]);
  ...
  const formatPackInfo = (ing: Ingredient) => {
    ...
    const packStr =
      up === 1 ? `${sizeStr}${ing.unit}` : `${up}×${sizeStr}${ing.unit}`;
      ...
      <div className="px-6 pt-6 pb-2 space-y-4">
        <StatCards
          stats={[
            {
              label: "Total Items",
              value: ingredients.filter((i) => i.active).length,
            },
            {
              label: "Low Stock",
              value: lowStockCount,
                ...
                filtered.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell
                        className={`text-right tabular-nums ${isLow ? "text-orange-600 font-medium" : ""}`}
                      >
                        {item.current_stock}
                        {isLow && (
                      ...
                      <TableCell className="text-right tabular-nums">
                        ...
                        {item.unit_cost_ex_base != null &&
                          item.unit_cost_ex_base > 0 && (
                            <span className="block text-xs text-muted-foreground">
                              {formatBaseUnitCost(
                                item.unit_cost_ex_base,
                      ...
                      <TableCell
                        className={`text-right tabular-nums ${hasWaste ? "text-amber-600" : ""}`}
                      >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, unitsPerPack: e.target.value })
                    }
                  />
                </div>
                <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {formatBaseUnitCost(
                    calculateCostPerBaseUnit(
                      ...
                      calculatePackToBaseFactor(
                        parseFloat(formData.unitsPerPack) || 1,
                        parseFloat(formData.unitSize) || 1,
                        formData.unit,
                      ),
                    ),
                    formData.unit === "kg" || formData.unit === "g"
                      ? "g"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, parLevel: e.target.value })
                  }
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, wastePercent: e.target.value })
                  }
                  placeholder="0"
                />
                {parseFloat(formData.wastePercent) > 0 &&
                  parseFloat(formData.cost) > 0 && (
                    <p className="text-xs text-amber-600 mt-1">
                      ...
                      {formatCurrency(
                        Math.round(
                          Math.round(parseFloat(formData.cost) * 100) /
                            (1 - parseFloat(formData.wastePercent) / 100),
                        ),
                      )}
                    </p>
                  )}
              ...
              <div className="flex items-end pb-2">
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox
                    ...
                    onCheckedChange={(v) =>
                      setFormData({ ...formData, gstApplicable: !!v })
                    }
                  />
                  GST applicable
                </label>
            ...
            <Button
              onClick={handleSave}
              className="w-full bg-brand hover:bg-brand-500 text-gray-900 font-semibold"
            >
