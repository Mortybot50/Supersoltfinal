...
function varianceBadge(status: ReconciliationLineItem['status']) {
  switch (status) {
    case 'received_full': return <Badge variant="default" className="bg-green-500 text-white text-xs">Received</Badge>
    case 'received_partial': return <Badge variant="secondary" className="bg-amber-100 text-amber-700 border-amber-300 text-xs">Partial</Badge>
    case 'not_received': return <Badge variant="destructive" className="text-xs">Not Received</Badge>
    case 'unexpected': return <Badge variant="outline" className="text-xs">Unexpected</Badge>
  }
}

function calcLineStatus(expected: number | null, received: number): ReconciliationLineItem['status'] {
...
export default function Reconciliation() {
  ...
  useEffect(() => {
    ...
    const initial: ReconLine[] = lineItems.map(li => {
      ...
      const poLine = selectedPO?.items?.find(pi => pi.ingredient_id === li.ingredient_id)
      ...
      return {
        ...
        receivedQuantity: (li.confirmed_quantity ?? li.extracted_quantity ?? '').toString(),
        expectedUnitPrice: poLine?.unit_cost ?? null,
        actualUnitPrice: li.confirmed_unit_price ?? li.extracted_unit_price ?? null,
        updateCost: false,
      }
    })
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const reconLineItems: ReconciliationLineItem[] = lines.map(l => {
        const qty = parseFloat(l.receivedQuantity) || 0
        const qtyVariance = l.expectedQuantity != null ? qty - l.expectedQuantity : null
        const priceVariance = (l.actualUnitPrice != null && l.expectedUnitPrice != null)
      ...
      const overallStatus: ReconciliationLog['status'] =
        reconLineItems.every(l => l.status === 'received_full')
          ? 'fully_received'
  ...
  const toolbar = (
    <PageToolbar
      ...
      filters={
        <Button variant="ghost" size="sm" onClick={() => navigate(`/inventory/invoices/${invoiceId}`)}>
      ...
      <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="text-sm text-muted-foreground">
            Invoice: <span className="font-medium text-foreground">{invoice.invoice_number ?? invoice.id.slice(0, 8)}</span>
          </div>
          <ArrowRight className="h-4 w-4 text-muted-foreground" />
          <div className="text-sm text-muted-foreground">
            Supplier: <span className="font-medium text-foreground">{invoice.supplier_name ?? '—'}</span>
          </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          {lines.map((line, idx) => {
            ...
            const qtyVariance = line.expectedQuantity != null ? received - line.expectedQuantity : null
            const qtyVariancePct = (line.expectedQuantity && line.expectedQuantity > 0)
              ? ((received - line.expectedQuantity) / line.expectedQuantity) * 100
              : null
            const priceVariance = (line.actualUnitPrice != null && line.expectedUnitPrice != null)
              ? line.actualUnitPrice - line.expectedUnitPrice
              : null
            const priceVariancePct = (line.expectedUnitPrice && line.expectedUnitPrice > 0 && priceVariance != null)
            ...
            return (
              <Card
                ...
                className={`${
                  status === 'not_received' ? 'border-destructive/40 bg-destructive/5' :
                  status === 'received_partial' ? 'border-amber-400/40 bg-amber-50/30 dark:bg-amber-950/10' :
                  status === 'received_full' ? 'border-green-400/30' : ''
                }`}
              >
                  ...
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-sm truncate">{line.ingredientName}</p>
                        {varianceBadge(status)}
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5 truncate">{line.rawDescription}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-6 text-sm flex-wrap">
                      ...
                      <div className="text-center min-w-[80px]">
                        <p className="text-xs text-muted-foreground mb-1">Expected (PO)</p>
                        <p className="font-medium">
                          {line.expectedQuantity != null ? line.expectedQuantity : '—'}
                        </p>
                      </div>
                      ...
                      {qtyVariance !== null && (
                        <div className="text-center min-w-[70px]">
                          <p className="text-xs text-muted-foreground mb-1">Qty Δ</p>
                          <p className={`font-medium ${varianceColor(qtyVariancePct)}`}>
                            {qtyVariance >= 0 ? '+' : ''}{qtyVariance.toFixed(2)}
                            {qtyVariancePct != null && (
                              <span className="text-xs ml-1">({qtyVariancePct.toFixed(1)}%)</span>
                            )}
                          </p>
                        </div>
                      )}
                    ...
                    <div className="flex items-center gap-4 text-sm flex-wrap">
                      <div className="text-center min-w-[80px]">
                        <p className="text-xs text-muted-foreground mb-1">PO Price</p>
                        <p>{line.expectedUnitPrice != null ? formatCurrency(line.expectedUnitPrice * 100) : '—'}</p>
                      </div>
                      <div className="text-center min-w-[80px]">
                        <p className="text-xs text-muted-foreground mb-1">Invoice Price</p>
                        <p className={priceVariancePct != null ? varianceColor(priceVariancePct) : ''}>
                          {line.actualUnitPrice != null ? formatCurrency(line.actualUnitPrice * 100) : '—'}
                        </p>
                      </div>

                      {priceVariance != null && Math.abs(priceVariance) > 0.005 && (
                        <div className="flex items-center gap-2">
                          {priceVariance > 0 ? (
                          ...
                          <div>
                            <p className={`text-xs font-medium ${varianceColor(priceVariancePct)}`}>
                              {priceVariance > 0 ? '+' : ''}{formatCurrency(priceVariance * 100)}
                              {priceVariancePct != null && ` (${priceVariancePct.toFixed(1)}%)`}
                            </p>
                            {line.ingredientId && (
                              <label className="flex items-center gap-1.5 text-xs cursor-pointer mt-1">
                                <input
                                  ...
                                  onChange={e => updateLine(idx, { updateCost: e.target.checked })}
                                  className="rounded"
                                />
                                Update cost
                              </label>
              ...
              <div className="space-y-1">
                <div className="flex gap-8">
                  <div>
                    ...
                    <p className="font-medium">{formatCurrency(totals.totalExpected * 100)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Received Value</p>
                    <p className="font-medium">{formatCurrency(totals.totalReceived * 100)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Variance</p>
                    <p className={`font-medium ${totals.variance === 0 ? 'text-green-600' : Math.abs(totals.variance) < 5 ? 'text-amber-600' : 'text-destructive'}`}>
                      {totals.variance >= 0 ? '+' : ''}{formatCurrency(totals.variance * 100)}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleConfirm} disabled={isSaving || lines.length === 0}>
            {isSaving ? (
              ...
              <><GitMerge className="h-4 w-4 mr-2" />Confirm Receipt</>
            )}
          </Button>
        </div>
      </div>
