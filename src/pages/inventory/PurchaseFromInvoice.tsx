  ...
  useEffect(() => {
    ...
    setLines(
      lineItems.map((li) => ({
        ...
        unitPrice: String(
          li.confirmed_unit_price ?? li.extracted_unit_price ?? "",
        ),
        included: li.match_status !== "unmatched",
        updateCost:
          li.match_status === "auto_matched" ||
          li.match_status === "manual_matched",
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const po: PurchaseOrder = {
        ...
        supplier_name:
          invoice.supplier_name ?? supplier?.name ?? "Unknown Supplier",
          ...
          <Card className="border-green-300 bg-green-50/40 dark:bg-green-950/20">
            <CardContent className="pt-4 flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span>
                  This invoice has already been converted to purchase{" "}
                  <span className="font-medium">{existingPO.po_number}</span>.
                </span>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() =>
                  navigate(`/inventory/purchase-orders/${existingPO.id}`)
                }
              >
                View Purchase
              </Button>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <Badge
                  variant={
                    invoice.status === "confirmed" ? "default" : "secondary"
                  }
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatCurrency(invoiceTotal * 100)}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatCurrency(matchedTotal * 100)}
                </span>
              </div>
              <Separator />
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-base">
              ...
              <span className="ml-2 text-sm font-normal text-muted-foreground">
                ({lines.filter((l) => l.included).length} of {lines.length}{" "}
                included)
              </span>
            </CardTitle>
              ...
              <Table>
                ...
                <TableBody>
                  {lines.map((line, idx) => {
                    const matchCfg =
                      MATCH_CONFIG[
                        line.matchStatus as keyof typeof MATCH_CONFIG
                      ] ?? MATCH_CONFIG.unmatched;
                    const qty = parseFloat(line.quantity) || 0;
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  updateLine(idx, {
                                    ingredientId: null,
                                    ingredientName: line.rawDescription,
                                    updateCost: false,
                                    updateStock: false,
                                  })
                                }
                            ...
                            <div className="flex flex-col gap-1">
                              <label className="flex items-center gap-1.5 text-xs cursor-pointer">
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) =>
                                    updateLine(idx, { updateStock: !!checked })
                                  }
                                  className="h-3 w-3"
                                />
                                Stock
                              ...
                              <label className="flex items-center gap-1.5 text-xs cursor-pointer">
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) =>
                                    updateLine(idx, { updateCost: !!checked })
                                  }
                                  className="h-3 w-3"
                                />
                                Cost
