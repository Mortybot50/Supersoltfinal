...
export default function ConnectPosStep({ orgId, onNext, onBack }: Props) {
  ...
  const handleConnect = () => {
    toast.success("Square POS", {
      description:
        "POS connection will be available after setup. You can connect later from Settings > Integrations.",
    });
  };
  ...
  return (
    <Card className="p-6">
      ...
      {connected ? (
        <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-lg mb-6">
          ...
          <div>
            ...
            {merchantName && (
              <p className="text-sm text-green-600">{merchantName}</p>
            )}
          </div>
        </div>
      ) : (
        <div className="text-center py-8 space-y-4">
          ...
          <Button onClick={handleConnect} size="lg" variant="outline">
            Connect Square POS
          </Button>
        </div>
      )}
      ...
      <div className="flex justify-between pt-6">
        ...
        <Button onClick={onNext}>{connected ? "Next" : "Skip for now"}</Button>
      </div>
    </Card>
  );
}
