...
import { calculateLineCost, getCompatibleUnits, calculatePackToBaseFactor, formatBaseUnitCost, calculateCostPerBaseUnit } from '@/lib/utils/unitConversions'
...
export default function RecipeEditor() {
  ...
  const costPerServe = useMemo(() => {
    return recipeForm.serves > 0 ? Math.round(totalBatchCost / recipeForm.serves) : 0
  ...
  const actualGpPercent = useMemo(() => {
    ...
    const priceExGst = linkedMenuItem.gst_mode === 'INC'
      ? Math.round(linkedMenuItem.price / 1.1)
      : linkedMenuItem.price
    return priceExGst > 0 ? ((priceExGst - costPerServe) / priceExGst) * 100 : 0
  ...
  const inheritedAllergens = useMemo(() => {
    ...
    for (const line of ingredients) {
      if (line.is_sub_recipe) {
        ...
        if (product?.allergens) product.allergens.forEach((a) => inherited.add(a))
      }
    }
    return inherited
  ...
  const wouldCreateCircularRef = useCallback((subRecipeId: string): boolean => {
    ...
    return subIngredients.some(
      (ri) => ri.is_sub_recipe && (ri.sub_recipe_id === recipeId || (ri.sub_recipe_id && wouldCreateCircularRef(ri.sub_recipe_id)))
    )
  ...
  const handleIngredientChange = (index: number, field: keyof RecipeIngredient, value: RecipeIngredient[keyof RecipeIngredient]) => {
    ...
    if (updated[index].is_sub_recipe && field === 'sub_recipe_id') {
      ...
      if (subRecipe) {
        ...
        updated[index].line_cost = subRecipe.cost_per_serve * updated[index].quantity
      ...
      updated[index].line_cost = Math.round((updated[index].unit_cost_ex_base || 0) * (updated[index].quantity || 0))
      ...
      if (product) {
        ...
        const ingredientCostPerBase = product.unit_cost_ex_base ||
          calculateCostPerBaseUnit(product.cost_per_unit, calculatePackToBaseFactor(
            product.units_per_pack || 1, product.unit_size || product.pack_size || 1, product.unit
          ))
        updated[index].unit_cost_ex_base = ingredientCostPerBase
        updated[index].line_cost = calculateLineCost(updated[index].quantity, updated[index].unit, ingredientCostPerBase)
        updated[index].cost_per_unit = ingredientCostPerBase
      }
    } else if ((field === 'quantity' || field === 'unit') && !updated[index].is_sub_recipe) {
  ...
  const handleRemoveStep = (index: number) => {
    setRecipeForm({ ...recipeForm, steps: recipeForm.steps.filter((_, i) => i !== index) })
  }
  ...
  const handleSave = async (status: 'draft' | 'published') => {
    ...
    if (recipeForm.serves <= 0) { toast.error('Servings must be greater than 0'); return }
    if (status === 'published' && !canPublish) {
      toast.error('Recipe must have ingredients and steps to publish'); return
    }

    setSaving(true)
    try {
      // Merge inherited allergens
      const allAllergens = [...new Set([...recipeForm.allergens, ...inheritedAllergens])]
      ...
      const recipeData: Recipe = {
        ...
        created_at: isNew ? new Date() : (existingRecipe?.created_at || new Date()),
        updated_at: new Date(),
        ...(status === 'published' && { published_at: new Date() }),
      }

      const ingredientsWithSortOrder = ingredients.map((ing, idx) => ({ ...ing, sort_order: idx }))
  ...
  const toolbar = (
    <PageToolbar
      ...
      actions={
        <>
          <Button variant="ghost" size="sm" onClick={() => navigate('/menu/recipes')}>
            <ArrowLeft className="h-4 w-4 mr-1" /> Recipes
          </Button>
          <Separator orientation="vertical" className="h-5" />
          {!isNew && existingRecipe && (
            <Badge variant={existingRecipe.status === 'published' ? 'default' : 'secondary'} className="text-xs">
          ...
          {!isNew && (
            <Button variant="ghost" size="sm" className="h-8 text-red-600" onClick={handleDelete}>
              <Trash2 className="h-3.5 w-3.5" />
            </Button>
          )}
          <Button variant="outline" size="sm" className="h-8" onClick={() => handleSave('draft')} disabled={saving}>
    ...
    <PageShell toolbar={toolbar}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-4">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              <div className="col-span-2">
                ...
                <Input value={recipeForm.name} onChange={(e) => setRecipeForm({ ...recipeForm, name: e.target.value })} placeholder="e.g., Margherita Pizza" />
              </div>
              <div>
                <Label className="text-xs">Category</Label>
                <Select value={recipeForm.category} onValueChange={(v: Recipe['category']) => setRecipeForm({ ...recipeForm, category: v })}>
              ...
              <div>
                ...
                <Input type="number" min="0.1" step="0.1" value={recipeForm.serves} onChange={(e) => setRecipeForm({ ...recipeForm, serves: parseFloat(e.target.value) || 1 })} />
              </div>
              <div>
                <Label className="text-xs">Waste %</Label>
                <Input type="number" min="0" max="99" value={recipeForm.wastage_percent} onChange={(e) => setRecipeForm({ ...recipeForm, wastage_percent: parseFloat(e.target.value) || 0 })} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-1">
                <Button size="sm" className="h-7 text-xs" onClick={handleAddIngredient}>
                  <Plus className="h-3 w-3 mr-1" /> Ingredient
                </Button>
                {availableSubRecipes.length > 0 && (
                  <Button size="sm" variant="outline" className="h-7 text-xs" onClick={handleAddSubRecipe}>
            ...
            {ingredients.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground text-sm">
                ...
                <Button variant="link" size="sm" onClick={handleAddIngredient}>Add first ingredient</Button>
              </div>
            ) : (
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {ingredients.map((ing, index) => (
                    <TableRow key={ing.id} className={ing.is_sub_recipe ? 'bg-blue-50/50 dark:bg-blue-950/20' : ''}>
                      <TableCell className="px-1">
                        <div className="flex flex-col gap-0.5">
                          <button className="p-0.5 hover:bg-muted rounded" onClick={() => handleMoveIngredient(index, 'up')} disabled={index === 0}>
                            <ChevronUp className="h-3 w-3" />
                          </button>
                          <button className="p-0.5 hover:bg-muted rounded" onClick={() => handleMoveIngredient(index, 'down')} disabled={index === ingredients.length - 1}>
                      ...
                      <TableCell>
                        {ing.is_sub_recipe ? (
                          <div>
                            <span className="text-xs text-blue-600 font-medium block mb-0.5">SUB-RECIPE</span>
                            <Select value={ing.sub_recipe_id || ''} onValueChange={(v) => handleIngredientChange(index, 'sub_recipe_id', v)}>
                              <SelectTrigger className="h-8"><SelectValue placeholder="Select recipe..." /></SelectTrigger>
                              <SelectContent>
                                {availableSubRecipes.map((r) => (
                                  <SelectItem key={r.id} value={r.id}>{r.name} ({formatCurrency(r.cost_per_serve)}/serve)</SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                          </div>
                        ) : (
                          <Select value={ing.product_id} onValueChange={(v) => handleIngredientChange(index, 'product_id', v)}>
                            <SelectTrigger className="h-8"><SelectValue placeholder="Select ingredient..." /></SelectTrigger>
                            <SelectContent>
                              {products.filter((p) => p.active).map((p) => (
                                <SelectItem key={p.id} value={p.id}>
                                  {p.name} ({p.unit}) — {formatCurrency(p.cost_per_unit)}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                      ...
                      <TableCell>
                        <Input
                          type="number" min="0" step={ing.is_sub_recipe ? '1' : '0.1'}
                          value={ing.quantity || ''}
                      ...
                      <TableCell>
                        {ing.is_sub_recipe ? (
                          ...
                          <Select value={ing.unit} onValueChange={(v: RecipeIngredient['unit']) => handleIngredientChange(index, 'unit', v)} disabled={!ing.product_id}>
                            <SelectTrigger className="h-8 w-20"><SelectValue /></SelectTrigger>
                            <SelectContent>
                              {getCompatibleUnits(ing.product_unit || 'g').map((u) => (
                      ...
                      <TableCell>
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => handleRemoveIngredient(index)}>
          ...
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button size="sm" className="h-7 text-xs" onClick={handleAddStep}><Plus className="h-3 w-3 mr-1" /> Step</Button>
            </div>
            {recipeForm.instructions !== undefined && (
              <div className="mb-3">
                <Label className="text-xs text-muted-foreground">General notes (optional)</Label>
                <Textarea value={recipeForm.instructions} onChange={(e) => setRecipeForm({ ...recipeForm, instructions: e.target.value })} placeholder="Tips, notes, overview..." rows={2} className="text-sm" />
              </div>
            )}
            <div className="space-y-2">
              {recipeForm.steps.map((step, index) => (
                <div key={index} className="flex gap-2 items-start">
                  ...
                  <Textarea value={step} onChange={(e) => handleStepChange(index, e.target.value)} placeholder={`Step ${index + 1}...`} rows={2} className="flex-1 text-sm" />
                  {recipeForm.steps.length > 1 && (
                    <Button variant="ghost" size="sm" className="h-7 w-7 p-0 mt-1" onClick={() => handleRemoveStep(index)}>
                      <X className="h-3.5 w-3.5" />
                    </Button>
                  )}
                </div>
          ...
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-4">
            ...
            {inheritedAllergens.size > 0 && (
              <div className="mb-3 p-2 bg-orange-50 dark:bg-orange-950/30 rounded text-xs text-orange-700 dark:text-orange-300">
                ...
                Auto-inherited from ingredients: {[...inheritedAllergens].map((a) => a.split(' (')[0]).join(', ')}
              </div>
            )}
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {COMMON_ALLERGENS.map((allergen) => {
                const isInherited = inheritedAllergens.has(allergen)
                const isChecked = recipeForm.allergens.includes(allergen) || isInherited
                return (
                  <label key={allergen} className={`flex items-center gap-1.5 text-xs cursor-pointer ${isInherited ? 'text-orange-700 font-medium' : ''}`}>
                    <Checkbox checked={isChecked} disabled={isInherited} onCheckedChange={() => !isInherited && toggleAllergen(allergen)} />
                    <span>{allergen.split(' (')[0]}</span>
                    {isInherited && <span className="text-[10px] text-orange-500">(auto)</span>}
                  </label>
                )
              })}
            </div>
        ...
        <div className="w-64 shrink-0 bg-slate-800 text-white p-4 flex flex-col overflow-y-auto">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Costing</h3>

          {/* Cost breakdown */}
          <div className="space-y-3">
            <div>
              <span className="text-xs text-slate-400">Ingredient Cost</span>
              <p className="text-lg font-bold">{formatCurrency(totalIngredientCost)}</p>
            </div>

            {recipeForm.wastage_percent > 0 && (
              <div>
                <span className="text-xs text-slate-400">+ Waste ({recipeForm.wastage_percent}%)</span>
                <p className="text-sm font-medium text-amber-400">{formatCurrency(wasteCost)}</p>
              </div>
            )}
            ...
            <div>
              ...
              <p className="text-xl font-bold">{formatCurrency(totalBatchCost)}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-teal-400">{formatCurrency(costPerServe)}</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <Input
                  ...
                  onChange={(e) => setRecipeForm({ ...recipeForm, gp_target_percent: parseFloat(e.target.value) || 65 })}
            ...
            <div>
              <span className="text-xs text-slate-400">Suggested Price (ex GST)</span>
              <p className="text-lg font-bold text-green-400">{formatCurrency(suggestedPrice)}</p>
              <span className="text-xs text-slate-500">{formatCurrency(suggestedPriceIncGst)} inc GST</span>
            </div>
          </div>
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium">{formatCurrency(linkedMenuItem.price)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-400">Actual GP%</span>
                  <span className={`text-sm font-bold ${actualGpPercent !== null ? getGpColor(actualGpPercent) : ''}`}>
                    {actualGpPercent !== null ? `${actualGpPercent.toFixed(1)}%` : '—'}
                  </span>
                </div>
              </div>
            </>
          ...
          <div className="space-y-1 text-xs text-slate-400">
            ...
            <div className="flex justify-between">
              <span>Allergens</span>
              <span>{new Set([...recipeForm.allergens, ...inheritedAllergens]).size}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Recipe Card — {recipeForm.name || 'Untitled'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-5 print:text-black">
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline">Cost/Serve: {formatCurrency(costPerServe)}</Badge>
            </div>

            {/* Allergens */}
            {(recipeForm.allergens.length > 0 || inheritedAllergens.size > 0) && (
              <div>
                <h3 className="font-semibold text-sm mb-1">Allergens</h3>
                <div className="flex flex-wrap gap-1">
                  {[...new Set([...recipeForm.allergens, ...inheritedAllergens])].map((a) => (
                    <Badge key={a} variant="destructive" className="text-xs">{a.split(' (')[0]}</Badge>
                  ))}
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-0.5 text-sm">
                {ingredients.filter((i) => i.product_id || i.is_sub_recipe).map((ing) => (
                  <li key={ing.id}>
                    <span className="font-medium">{ing.quantity} {ing.is_sub_recipe ? 'serve(s)' : ing.unit}</span>{' '}
                    {ing.product_name}
                    {ing.is_sub_recipe && <span className="text-xs text-muted-foreground ml-1">(sub-recipe)</span>}
                  </li>
                ))}
              </ul>
            </div>
            ...
            {recipeForm.steps.filter((s) => s.trim()).length > 0 && (
              <div>
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm">
                  {recipeForm.steps.filter((s) => s.trim()).map((step, i) => <li key={i}>{step}</li>)}
                </ol>
              </div>
            )}
