...
export default function Compliance() {
  ...
  const { staff, onboardingSteps, onboardingDocuments, onboardingInvites } =
    useDataStore();
  ...
  const onboardingMetrics = useMemo(() => {
    const byStatus = {
      not_started: allStaff.filter((s) => s.onboarding_status === "not_started")
        .length,
      invited: allStaff.filter((s) => s.onboarding_status === "invited").length,
      in_progress: allStaff.filter((s) => s.onboarding_status === "in_progress")
        .length,
      pending_review: allStaff.filter(
        (s) => s.onboarding_status === "pending_review",
      ).length,
      roster_ready: allStaff.filter(
        (s) => s.onboarding_status === "roster_ready",
      ).length,
    };
  ...
  const documentCompliance = useMemo(() => {
    ...
    const issues: {
      ...
      staffName: string;
      docType: string;
      status: "missing" | "pending" | "expired";
    }[] = [];
    ...
    const totalRequired =
      allStaff.filter((s) => s.onboarding_status !== "not_started").length *
      requiredTypes.length;
    const totalProvided = onboardingDocuments.filter(
      (d) =>
        requiredTypes.some((rt) => rt.value === d.document_type) &&
        d.status === "approved",
    ).length;
    const docComplianceRate =
      totalRequired > 0
        ? Math.round((totalProvided / totalRequired) * 100)
        : 100;
    ...
    return allStaff
      ...
      .map((s) => {
        ...
        const isExpired = invite
          ...
          : false;
        const daysSinceInvite = invite
          ? differenceInDays(new Date(), new Date(invite.sent_at))
          : null;
      ...
      .sort((a, b) => {
        ...
        return (
          (order[a.staff.onboarding_status] ?? 5) -
          (order[b.staff.onboarding_status] ?? 5)
        );
      });
    ...
    return allStaff
      ...
      .map((s) => {
        ...
        const hasID = docs.some(
          (d) => d.document_type === "id_proof" && d.status === "approved",
        );
        const total = 4;
        const complete = [hasTFN, hasSuper, hasBank, hasID].filter(
          Boolean,
        ).length;
    ...
    <PageToolbar
      ...
      actions={
        <Button
          ...
          size="sm"
          className="h-8"
          onClick={() => navigate("/workforce/people")}
        >
      ...
      <div className="px-4 pt-4 space-y-3">
        <StatCards
          stats={[
            { label: "Active Staff", value: onboardingMetrics.totalActive },
            {
              ...
              value: `${onboardingMetrics.completionRate}%`,
            },
          ]}
          columns={2}
        />
        <SecondaryStats
          stats={[
            {
              label: "Roster Ready",
              value: onboardingMetrics.byStatus.roster_ready,
            },
            {
              label: "Pending Review",
              value: onboardingMetrics.byStatus.pending_review,
            },
            {
              label: "In Progress",
              value: onboardingMetrics.byStatus.in_progress,
            },
            { label: "Invited", value: onboardingMetrics.byStatus.invited },
            {
              label: "Not Started",
              value: onboardingMetrics.byStatus.not_started,
            },
          ]}
        />
      </div>
      <div className="p-4 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card className="p-4">
            ...
            <p className="text-2xl font-bold">
              {onboardingMetrics.byStatus.roster_ready}
            </p>
            <p className="text-xs text-muted-foreground">
              {onboardingMetrics.completionRate}% of active
            </p>
          </Card>
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-2xl font-bold">
              {onboardingMetrics.byStatus.pending_review}
            </p>
          </Card>
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-2xl font-bold">
              {documentCompliance.docComplianceRate}%
            </p>
            <p className="text-xs text-muted-foreground">
              {documentCompliance.totalProvided}/
              {documentCompliance.totalRequired} verified
            </p>
          </Card>
        </div>
                    ...
                    {staffOnboarding.map(
                      ({
                        ...
                        <TableRow
                          ...
                          onClick={() => navigate(`/workforce/people/${s.id}`)}
                        >
                          <TableCell>
                          ...
                          <TableCell>
                            <Badge
                              className={
                                ONBOARDING_STATUS_COLORS[s.onboarding_status] ||
                                ""
                              }
                            >
                          ...
                          <TableCell className="text-sm text-muted-foreground">
                            {invite
                              ? format(new Date(invite.sent_at), "dd MMM")
                              : "—"}
                          </TableCell>
                          <TableCell>
                            {daysSinceInvite !== null && (
                              <span
                                className={`text-sm ${daysSinceInvite > 5 ? "text-red-500 font-medium" : "text-muted-foreground"}`}
                              >
                                {daysSinceInvite}d
                              </span>
                            )}
                      ...
                      <TableRow key={`${issue.staffId}-${issue.docType}-${i}`}>
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            variant="ghost"
                            onClick={() =>
                              navigate(`/workforce/people/${issue.staffId}`)
                            }
                          >
                            View Profile
                          </Button>
          ...
          <TabsContent value="checklist">
            {staffCompliance.length === 0 ? (
              <Card className="p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Staff who complete onboarding will appear here
                </p>
              </Card>
            ) : (
              <Card>
                      ...
                      ({
                        ...
                        >
                          ...
                          <TableCell className="text-center">
                            <Badge
                              variant={
                                complete === total ? "default" : "secondary"
                              }
                            >
                              {complete}/{total}
                            </Badge>
