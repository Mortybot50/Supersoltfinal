...
export function InvoiceReviewModal() {
  ...
  const lineTotal = selectedJob.lines_json.reduce(
    (sum, line) => sum + line.ext_price,
    0,
  );
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="font-medium">
                    {selectedJob.header_json.supplier_name}
                  </p>
                  {selectedJob.header_json.abn && (
                    <p className="text-xs text-muted-foreground mt-1">
                      ABN: {selectedJob.header_json.abn}
                    </p>
                  )}
                </div>
                <Badge
                  variant={
                    selectedJob.supplier_confidence > 0.8
                      ? "default"
                      : "secondary"
                  }
                >
            ...
            <Card className="p-4">
              <div>
                ...
                <p className="font-medium font-mono">
                  {selectedJob.header_json.invoice_number}
                </p>
                <p className="text-sm text-muted-foreground mt-1">
                  {format(
                    new Date(selectedJob.header_json.invoice_date),
                    "dd MMM yyyy",
                  )}
                </p>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1 mt-1">
                  <div className="flex justify-between text-sm">
                    <span>Subtotal:</span>
                    <span>
                      {formatCurrency(selectedJob.header_json.subtotal * 100)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>GST:</span>
                    <span>
                      {formatCurrency(selectedJob.header_json.gst * 100)}
                    </span>
                  </div>
                  <div className="flex justify-between font-medium border-t pt-1">
                    <span>Total:</span>
                    <span>
                      {formatCurrency(selectedJob.header_json.total * 100)}
                    </span>
                  </div>
          ...
          {Math.abs(variancePercent) > 0.5 && (
            <Card
              className={`p-4 ${Math.abs(variancePercent) > 2 ? "border-destructive" : "border-yellow-500"}`}
            >
              <div className="flex items-start gap-3">
                <AlertTriangle
                  className={`h-5 w-5 ${Math.abs(variancePercent) > 2 ? "text-destructive" : "text-yellow-500"}`}
                />
                <div className="flex-1">
                  <p className="font-medium">Totals Variance Detected</p>
                  <p className="text-sm text-muted-foreground mt-1">
                    Line total differs by {variance > 0 ? "+" : ""}
                    ...
                    return (
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-right">
                          {formatCurrency(line.unit_price * 100)}
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          {mapping.unit_cost_computed ? (
                            <div className="text-sm">
                              <p className="font-medium">
                                {formatCurrency(mapping.unit_cost_computed)}/
                                {mapping.unit}
                              </p>
                            </div>
                          ) : (
          ...
          <div className="flex items-center justify-between pt-4 border-t">
            <Button
              variant="outline"
              onClick={() => saveDraft(selectedJob.id)}
            ...
            <div className="flex gap-2">
              ...
              <Button
                onClick={() => approveInvoice(selectedJob.id)}
