...
export function InvoicesTab() {
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      ...
      const matchesSearch =
        !searchQuery ||
        job.header_json.invoice_number
          .toLowerCase()
          .includes(searchQuery.toLowerCase()) ||
        job.header_json.supplier_name
          .toLowerCase()
          .includes(searchQuery.toLowerCase());
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            onClick={() =>
              useInvoiceIntakeStore.setState({ uploadDrawerOpen: true })
            }
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold">
            {formatCurrency(spendMetrics.thisMonth)}
          </p>
          {spendMetrics.monthChange !== 0 && (
            <p
              className={`text-xs ${spendMetrics.monthChange > 0 ? "text-red-500" : "text-green-500"}`}
            >
              {spendMetrics.monthChange > 0 ? "+" : ""}
              {spendMetrics.monthChange.toFixed(1)}% vs last month
            </p>
          )}
        </Card>
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold">
            {formatCurrency(spendMetrics.lastMonth)}
          </p>
        </Card>
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-1">
        ...
        <Card className="p-4">
          ...
          <p className="text-xs text-muted-foreground">
            {formatCurrency(spendMetrics.totalApproved)} all-time
          </p>
        </Card>
      </div>
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2">
            {spendMetrics.topSuppliers.map(([name, spend]) => {
              const pct =
                spendMetrics.totalApproved > 0
                  ? (spend / spendMetrics.totalApproved) * 100
                  : 0;
              return (
            ...
            {filteredJobs.length === 0 ? (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-12">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <Button
                      onClick={() =>
                        useInvoiceIntakeStore.setState({
              ...
              filteredJobs.map((job) => (
                ...
                >
                  <TableCell>
                    {format(
                      new Date(job.header_json.invoice_date || job.created_at),
                      "dd MMM yyyy",
                    )}
                  </TableCell>
                  <TableCell>
