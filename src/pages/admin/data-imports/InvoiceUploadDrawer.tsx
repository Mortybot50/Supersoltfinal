...
import { Upload, X, FileText, Image as ImageIcon, Plus, Trash2, Link2 } from 'lucide-react'
...
export function InvoiceUploadDrawer() {
  const { uploadDrawerOpen, isProcessing, uploadInvoice, jobs } = useInvoiceIntakeStore()
  ...
  const filteredSuppliers = useMemo(() => {
    ...
    return suppliers
      .filter((s) => s.active && (s.name.toLowerCase().includes(q) || s.abn?.includes(q)))
      .slice(0, 10)
  ...
  const supplierPOs = useMemo(() => {
    ...
    return purchaseOrders
      .filter((po) => po.supplier_id === manualSupplierId && (po.status === 'submitted' || po.status === 'confirmed'))
      .slice(0, 20)
  }, [purchaseOrders, manualSupplierId])

  // Calculate totals
  const manualSubtotal = manualLines.reduce((sum, l) => sum + l.qty * l.unit_price, 0)
  const manualGst = manualGstMode === 'NONE' ? 0 : manualGstMode === 'INC' ? manualSubtotal / 11 : manualSubtotal * 0.1
  const manualTotal = manualGstMode === 'NONE' ? manualSubtotal : manualGstMode === 'INC' ? manualSubtotal : manualSubtotal + manualGst
  ...
  const handleManualSubmit = async () => {
    ...
    if (isDuplicate) {
      toast.error('Duplicate invoice — this invoice number already exists for this supplier')
      return
    }
    const validLines = manualLines.filter((l) => l.description.trim() && l.qty > 0 && l.unit_price > 0)
    ...
    const newJob = {
      ...
      header_json: {
        ...
        subtotal: manualGstMode === 'INC' ? manualSubtotal - manualGst : manualSubtotal,
        gst: manualGst,
        total: manualTotal,
      },
  ...
  const addLine = () => {
    setManualLines([...manualLines, { description: '', qty: 1, unit_price: 0 }])
  }
    ...
    <Sheet open={uploadDrawerOpen} onOpenChange={(open) => { if (!open) handleClose() }}>
      <SheetContent className="sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>Upload a file or enter invoice details manually</SheetDescription>
        </SheetHeader>
            ...
            <TabsContent value="manual" className="space-y-4 mt-4">
                    ...
                    <div>
                      ...
                      {selectedSupplier.abn && (
                        <p className="text-xs text-muted-foreground font-mono">
                          ABN {selectedSupplier.abn.replace(/(\d{2})(\d{3})(\d{3})(\d{3})/, '$1 $2 $3 $4')}
                        </p>
                      )}
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => setManualSupplierId('')}>
                      <X className="h-3 w-3" />
                    </Button>
                  </div>
                ) : (
                    ...
                    {filteredSuppliers.length > 0 && (
                      <div className="border rounded-md max-h-32 overflow-y-auto">
                        {filteredSuppliers.map((s) => (
                          ...
                          >
                            {s.name}
                            {s.abn && <span className="text-xs text-muted-foreground ml-2">ABN {s.abn}</span>}
                          </button>
                        ))}
                      </div>
                    )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  />
                  {isDuplicate && (
                    <p className="text-xs text-red-500 font-medium">Duplicate invoice detected</p>
                  )}
                </div>
                <div className="space-y-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label>GST</Label>
                  <Select value={manualGstMode} onValueChange={(v) => setManualGstMode(v as GSTMode)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1">
                  ...
                  <Select value={manualPoNumber || '_none'} onValueChange={(v) => setManualPoNumber(v === '_none' ? '' : v)}>
                    <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {manualLines.map((line, idx) => (
                    <div key={idx} className="grid grid-cols-[1fr_60px_80px_28px] gap-1.5 items-end">
                      <div>
              ...
              <div className="bg-muted rounded-md p-3 space-y-1 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">${(manualGstMode === 'INC' ? manualSubtotal - manualGst : manualSubtotal).toFixed(2)}</span>
                </div>
                {manualGstMode !== 'NONE' && (
                  <div className="flex justify-between">
              ...
              <Button onClick={handleManualSubmit} disabled={isProcessing} className="w-full">
                {manualPoNumber ? (
                  <>
