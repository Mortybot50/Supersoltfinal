...
export default function Integrations() {
  ...
  const loadSquare = useCallback(async () => {
    ...
    try {
      const { data: conn } = await supabase
        ...
        .select(
          "id, provider, merchant_id, merchant_name, is_active, last_sync_at, last_sync_status, created_at",
        )
  ...
  const loadXero = useCallback(async () => {
    ...
    try {
      const { data: conn } = (await supabase
        ...
        .select(
          "id, tenant_id, tenant_name, is_active, last_sync_at, last_sync_status, created_at",
        )
        .eq("org_id", currentOrg.id)
        .eq("is_active", true)
    ...
    if (connected === "square") {
      ...
      const messages: Record<string, string> = {
        ...
        tenants_fetch_failed:
          "Connected but could not retrieve Xero organisation.",
  ...
  const handleSquareDisconnect = async () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Disconnect request failed",
      );
    } finally {
  ...
  const handleXeroSync = async () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Xero sync request failed",
      );
    } finally {
  ...
  const squareError =
    squareConnected && squareConn?.last_sync_status?.startsWith("error");
  const activeLocation = squareLocations.find((l) => l.is_active);

  const xeroConnected = xeroConn?.is_active === true;
  const xeroError =
    xeroConnected && xeroConn?.last_sync_status?.startsWith("error");
  ...
  return (
    <PageShell toolbar={toolbar}>
          ...
          <Card className="flex flex-col">
            ...
            <CardContent className="flex-1 flex flex-col">
              {squareLoading ? (
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {squareConn?.merchant_id && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono text-xs">
                          {squareConn.merchant_id.slice(0, 12)}...
                        </span>
                      </div>
                    )}
                    {squareConn?.last_sync_at && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {formatDistanceToNow(
                            new Date(squareConn.last_sync_at),
                            { addSuffix: true },
                          )}
                        </span>
                      </div>
                ...
                <div className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    Automatically sync sales, orders, and payment data from
                    Square
                  </p>
                  <div className="space-y-1.5">
                    ...
                    {["Orders", "Payments", "Line Items", "Refunds"].map(
                      (item) => (
                        <div
                          key={item}
          ...
          <Card className="flex flex-col">
            ...
            <CardContent className="flex-1 flex flex-col">
              {xeroLoading ? (
                ...
                <div className="space-y-4">
                  <div className="space-y-2 text-sm">
                    ...
                    {xeroConn?.last_sync_at && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {formatDistanceToNow(
                            new Date(xeroConn.last_sync_at),
                            { addSuffix: true },
                          )}
                        </span>
                      </div>
                  ...
                  <div className="flex gap-2 mt-auto pt-2">
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        navigate("/admin/integrations/xero/mappings")
                      }
                    >
                ...
                <div className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    Sync invoices, purchase orders, payroll, and chart of
                    accounts with Xero
                  </p>
                  <div className="space-y-1.5">
                    <p className="text-xs font-medium text-muted-foreground">
              ...
              <Card key={integration.id} className="flex flex-col opacity-75">
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-lg text-white flex items-center justify-center ${integration.color}`}
                      >
      ...
      <Dialog
        open={confirmSquareDisconnect}
        onOpenChange={setConfirmSquareDisconnect}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Disconnect Square POS?</DialogTitle>
            <DialogDescription>
              This will stop syncing sales data from Square. Your existing
              synced data will be preserved.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setConfirmSquareDisconnect(false)}
            >
              Cancel
            </Button>
            <Button
      ...
      <Dialog
        open={confirmXeroDisconnect}
        onOpenChange={setConfirmXeroDisconnect}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Disconnect Xero?</DialogTitle>
            <DialogDescription>
              This will stop syncing financial data with Xero. Your existing
              synced data in Xero will be preserved. Account mappings will also
              be retained for when you reconnect.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setConfirmXeroDisconnect(false)}
            >
              Cancel
            </Button>
            <Button
