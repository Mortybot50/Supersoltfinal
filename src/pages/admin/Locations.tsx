...
export default function Locations() {
  ...
  const [editingSchedule, setEditingSchedule] = useState<CountSchedule | null>(
    null,
  );
  ...
  const loadAssignments = async (locationId: string) => {
    try {
      ...
      const mapped = (data || []).map(
        (a: Assignment & { ingredients?: { name: string } }) => ({
          ...a,
  ...
  const handleSaveLocation = async () => {
    try {
      ...
      if (
        ...
        !locationForm.temperature_target_c
      ) {
        toast.error("Validation Error", {
          description:
            "Temperature target (°C) is required for fridges and freezers",
          variant: "destructive",
        });
        return;
      }
    ...
    try {
      ...
      if (
        ...
        toast.error("Cannot Delete", {
          description:
            "This location has bins or assignments. Remove them first.",
          variant: "destructive",
        });
        return;
      }
  ...
  const handleAddBin = async () => {
    ...
    try {
      const { error } = await supabase.from("inv_bins").insert([
        {
      ...
      toast.error("Error", {
        description:
          error instanceof Error ? error.message : "Failed to add bin",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSaveSchedule = async () => {
    try {
      if (
        ...
        scheduleForm.schedule_name.trim() === ""
      ) {
        toast.error("Validation Error", {
      ...
      if (
        !scheduleForm.location_ids ||
        scheduleForm.location_ids.length === 0
      ) {
        toast.error("Validation Error", {
          description: "Select at least one location",
        });
        return;
      }
  ...
  const filteredLocations = locations.filter((loc) => {
    ...
    const matchesStatus =
      statusFilter === "all" ||
      (statusFilter === "active" ? loc.is_active : true);
  ...
  const stats = {
    ...
    coldStorage: locations.filter(
      (l) => l.type === "Fridge" || l.type === "Freezer",
    ).length,
                        ...
                        <TableCell>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem
                                onClick={() =>
                                  handleOpenLocationDialog(location)
                                }
                              >
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-muted-foreground">
                            {schedule.frequency} •{" "}
                            {schedule.location_ids.length} locations
                          </div>
                        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  POS, printers, suppliers, and other venue settings are managed
                  in Venue Settings.
                </p>
              </CardContent>
            </Card>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <Tabs defaultValue="general" className="mt-4">
                ...
                <div className="grid gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="Storeroom">Storeroom</SelectItem>
                        <SelectItem value="LineStation">
                          Line Station
                        </SelectItem>
                        <SelectItem value="Fridge">Fridge</SelectItem>
                        <SelectItem value="Freezer">Freezer</SelectItem>
                        <SelectItem value="ReceivingBay">
                          Receiving Bay
                        </SelectItem>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setLocationForm({
                          ...locationForm,
                          counting_method: value as CountingMethod,
                        })
                      }
                    >
                      <SelectTrigger>
                  ...
                  {(locationForm.type === "Fridge" ||
                    locationForm.type === "Freezer") && (
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setLocationForm({
                            ...locationForm,
                            temperature_target_c: e.target.value
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setLocationForm({
                          ...locationForm,
                          capacity_hint: e.target.value || null,
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setLocationForm({ ...locationForm, is_active: checked })
                      }
                    />
              ...
              <TabsContent value="bins" className="space-y-4 mt-4">
                {!editingLocation ? (
                  <p className="text-sm text-muted-foreground">
                    Save the location first to add bins.
                  </p>
                ) : (
                  <>
                    <Table>
              ...
              <TabsContent value="assignments" className="space-y-4 mt-4">
                {!editingLocation ? (
                  <p className="text-sm text-muted-foreground">
                    Save the location first to add assignments.
                  </p>
                ) : (
                  <>
                    <Table>
                        ...
                        {assignments.length === 0 ? (
                          ...
                          assignments.map((assignment) => (
                            <TableRow key={assignment.id}>
                              ...
                              <TableCell>
                                {assignment.par_level || "-"}
                              </TableCell>
                              <TableCell>
                                {assignment.min_level || "-"}
                              </TableCell>
                              <TableCell>
                                {assignment.max_level || "-"}
                              </TableCell>
                              <TableCell>
                                <Button
                                  size="icon"
                    ...
                    <p className="text-sm text-muted-foreground">
                      Assign ingredients to this location from the Ingredients
                      page.
                    </p>
                  </>
                )}
              ...
              <TabsContent value="devices" className="space-y-4 mt-4">
                {!editingLocation ? (
                  <p className="text-sm text-muted-foreground">
                    Save the location first to add devices.
                  </p>
                ) : (
                  <>
                    <Table>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Device assignment UI coming soon.
                    </p>
                  </>
                )}
              </TabsContent>
        ...
        <Dialog open={scheduleDialogOpen} onOpenChange={setScheduleDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Configure count schedule details and locations.
              </DialogDescription>
            </DialogHeader>
                  ...
                  {locations
                    ...
                    .map((location) => (
                      ...
                      >
                        <Checkbox
                          id={`loc-${location.id}`}
                          checked={scheduleForm.location_ids?.includes(
                            location.id,
                          )}
                          onCheckedChange={(checked) => {
