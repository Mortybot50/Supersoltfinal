...
export default function People() {
  ...
  const activeStaff = staffList.filter(
    (s) => s.status === "active" && s.onboarding_status === "roster_ready",
  );
  ...
  const filteredActiveStaff = useMemo(() => {
    ...
    return activeStaff
      .filter((s) => {
        ...
        const matchEmp =
          filterEmpType === "all" || s.employment_type === filterEmpType;
  ...
  const handleSendInvite = async () => {
    ...
    const newStaff: Staff = {
      ...
      hourly_rate: inviteForm.hourly_rate
        ? Math.round(parseFloat(inviteForm.hourly_rate) * 100)
        : 0,
      start_date: new Date(),
      status: "active",
          ...
          <TableBody>
            {tableStaff.length === 0 ? (
              ...
              tableStaff.map((person) => {
                const upcoming = rosterMetrics.getUpcomingShiftsForStaff(
                  person.id,
                );
                  ...
                  <TableRow key={person.id}>
                    <TableCell>
                      <div
                        ...
                        onClick={() =>
                          navigate(`/workforce/people/${person.id}`)
                        }
                      >
                        <div
                    ...
                    <TableCell>
                      <StatusBadge
                        status={
                          person.role as "manager" | "supervisor" | "crew"
                        }
                      />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {nextShift ? (
                        <div className="text-xs text-muted-foreground">
                          <div>
                            {format(new Date(nextShift.date), "EEE d MMM")}
                          </div>
                          <div>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {person.status === "active" ? (
                            ...
                            <DropdownMenuItem
                              onClick={() => handleActivateStaff(person.id)}
                            >
                              Activate
                            </DropdownMenuItem>
                          )}
  ...
  const pendingReviewCount = invitedStaff.filter(
    (s) => s.staff.onboarding_status === "pending_review",
  ).length;
      ...
      filters={
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Select value={filterRole} onValueChange={setFilterRole}>
            <SelectTrigger className="h-9 w-[120px] text-sm border-border/60">
          ...
          <Select value={filterEmpType} onValueChange={setFilterEmpType}>
            <SelectTrigger className="h-9 w-[130px] text-sm border-border/60">
          ...
          <Select
            value={sortBy}
            onValueChange={(v) => setSortBy(v as "name" | "start_date")}
          >
            <SelectTrigger className="h-9 w-[130px] text-sm border-border/60">
              <SelectValue />
            </SelectTrigger>
  ...
  return (
    <PageShell toolbar={toolbar}>
      <div className="px-6 pt-6 pb-2 space-y-4">
        ...
        />
        <SecondaryStats
          stats={[{ label: "Pending Review", value: pendingReviewCount }]}
        />
      </div>
        ...
        <Tabs defaultValue="active">
          <TabsList className="mb-4">
            ...
            <TabsTrigger value="invitations">
              Invitations ({invitedStaff.length})
            </TabsTrigger>
            ...
            {invitedStaff.length === 0 ? (
              ...
              <div className="rounded-xl border border-border/60 bg-card shadow-sm overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {invitedStaff.map(
                      ({ staff: person, invite, completedSteps }) => {
                        const statusLabel = person.onboarding_status.replace(
                          /_/g,
                          " ",
                        );
                        const isExpired =
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  ...
                                  {invite && (
                                    <DropdownMenuItem
                                      onClick={() =>
                                        handleCopyInviteUrl(invite.token)
                                      }
                                    >
                                  ...
                                  {person.onboarding_status ===
                                    ...
                                    <DropdownMenuItem
                                      onClick={() => {
                                        ...
                                        toast.success(
                                          `${person.name} approved and roster-ready`,
                                        );
                                      }}
                                    >
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setInviteErrors((prev) => ({ ...prev, first_name: "" }));
                    }}
                    placeholder="Jane"
                    className={
                      inviteErrors.first_name ? "border-destructive" : ""
                    }
                  />
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setInviteForm({
                        ...inviteForm,
                        role: v as "manager" | "supervisor" | "crew",
                      })
                    }
                  >
                    <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setInviteErrors((prev) => ({ ...prev, hourly_rate: "" }));
                  }}
                  placeholder="28.50"
                  className={
                    inviteErrors.hourly_rate ? "border-destructive" : ""
                  }
                />
                {inviteErrors.hourly_rate && (
                  <p className="text-sm text-destructive mt-1">
                    {inviteErrors.hourly_rate}
                  </p>
                )}
