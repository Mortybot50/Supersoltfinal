...
export default function DocumentsStep({
  ...
  const staffDocuments = onboardingDocuments.filter(
    (doc) => doc.staff_id === staffId,
  );
  ...
  const requiredDocsUploaded = DOCUMENT_TYPES.filter((dt) => dt.required).every(
    (dt) => staffDocuments.some((doc) => doc.document_type === dt.value),
  );
  ...
  return (
    <Card className="p-6">
      ...
      <p className="text-muted-foreground mb-6">
        Please upload the required documents. All files are securely stored and
        encrypted.
      </p>
      ...
      <div className="space-y-6">
        {DOCUMENT_TYPES.map((docType) => {
          const existingDoc = staffDocuments.find(
            (doc) => doc.document_type === docType.value,
          );
