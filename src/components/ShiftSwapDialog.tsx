    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          target_staff_id:
            targetStaffId && targetStaffId !== "open" ? targetStaffId : null,
          notes: notes || null,
        })
        .select()
        .single();
      ...
      const targetStaffMember =
        targetStaffId && targetStaffId !== "open"
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Leave as "Anyone Available" to let any team member claim this
              shift
            </p>
          </div>
...
export function SwapRequestsPanel({
  ...
  const pendingRequests = shiftSwapRequests.filter(
    (r) => r.status === "pending",
  );
  ...
  const getShift = (shiftId: string) =>
    rosterShifts.find((s) => s.id === shiftId);
