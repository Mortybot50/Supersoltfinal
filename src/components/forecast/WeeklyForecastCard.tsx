...
import type { ConfidenceLevel, DailyForecast } from '@/lib/services/forecastService'
    ...
    <div className="bg-background border rounded-lg shadow-lg p-3 text-sm">
      ...
      {payload.map((entry) => (
        <p key={entry.dataKey} style={{ color: entry.color }}>
          {entry.dataKey === 'forecast' ? 'Forecast' : 'Actual'}: {fmtDollarsDecimal(entry.value)}
        </p>
      ))}
    </div>
  )
...
export function WeeklyForecastCard({ venueId, className }: WeeklyForecastCardProps) {
  ...
  const totalForecast = useMemo(() => {
    ...
    return forecast.reduce((s: number, d: DailyForecast) => s + d.forecastedRevenue, 0)
  }, [forecast])

  const totalActual = useMemo(() => {
    if (!forecast) return null
    const daysWithActual = forecast.filter((d: DailyForecast) => d.actualRevenue !== null)
    if (daysWithActual.length === 0) return null
    return daysWithActual.reduce((s: number, d: DailyForecast) => s + (d.actualRevenue ?? 0), 0)
  }, [forecast])

  const avgConfidence = useMemo<ConfidenceLevel>(() => {
    if (!forecast || forecast.length === 0) return 'low'
    const scores = { low: 1, medium: 2, high: 3 }
    const avg = forecast.reduce((s: number, d: DailyForecast) => s + scores[d.confidence], 0) / forecast.length
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-medium">Weekly Sales Forecast</CardTitle>
          </div>
          <div className="flex items-center gap-2">
            <Badge variant="outline" className={confidenceColors[avgConfidence]}>
          ...
          <>
            ...
            <div className="flex items-center gap-6 mb-4">
              <div>
                ...
                <p className="text-xl font-bold text-teal-600">{fmtDollars(totalForecast)}</p>
              </div>
              {totalActual !== null && (
                <div>
                  <p className="text-xs text-muted-foreground">Actual (so far)</p>
                  <p className="text-xl font-bold text-blue-500">{fmtDollars(totalActual)}</p>
                </div>
              )}
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={chartData} barGap={2}>
                ...
                />
                <Bar dataKey="forecast" name="Forecast" fill={FORECAST_COLOR} radius={[2, 2, 0, 0]} opacity={0.7} />
            ...
            <div className="flex gap-1 mt-3 justify-between">
              {forecast?.map((d: DailyForecast) => (
                <div key={d.date} className="text-center flex-1">
                  <p className="text-[10px] text-muted-foreground">{d.dayLabel.slice(0, 3)}</p>
                  <p className="text-xs font-medium">{fmtDollars(d.forecastedRevenue)}</p>
                  <div className={`mx-auto mt-1 w-2 h-2 rounded-full ${
                    d.confidence === 'high' ? 'bg-green-500' :
                    d.confidence === 'medium' ? 'bg-amber-500' : 'bg-red-500'
                  }`} />
                </div>
              ))}
            </div>
            ...
            {avgConfidence === 'low' && (
              <div className="flex items-center gap-2 mt-3 p-2 rounded bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400 text-xs">
                ...
                <span>Forecast accuracy improves with more data. 4+ weeks recommended.</span>
              </div>
            )}
          </>
        )}
