    ...
    <div className="bg-background border rounded-lg shadow-lg p-3 text-sm">
      ...
      {payload.map((entry) => (
        <p key={entry.dataKey} style={{ color: entry.color }}>
          {entry.dataKey === 'forecasted' ? 'Forecast' : 'Actual'}: {fmtDollars(entry.value)}
        </p>
      ))}
    </div>
  )
...
export function ForecastAccuracy({ venueId, className }: ForecastAccuracyProps) {
  const { data: report, isLoading: reportLoading } = useLastWeekAccuracy(venueId)
        ...
        <div className="flex items-center justify-between">
          ...
          {report && mapeInfo && (
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs ${mapeInfo.color}`}>{mapeInfo.text}</span>
            </div>
          )}
        </div>
      </CardHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <p className="text-sm font-semibold">{fmtDollars(report.totalForecasted)}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Total Actual</p>
                <p className="text-sm font-semibold">{fmtDollars(report.totalActual)}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Variance</p>
                <p className={`text-sm font-semibold ${
                  Math.abs(report.overallVariance) <= 10 ? 'text-green-600' :
                  Math.abs(report.overallVariance) <= 20 ? 'text-amber-600' : 'text-red-600'
                }`}>
                  {report.overallVariance > 0 ? '+' : ''}{report.overallVariance.toFixed(1)}%
                </p>
              </div>
            </div>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={chartData} barGap={2}>
                ...
                <Bar dataKey="forecasted" name="Forecast" fill="#0d9488" radius={[2, 2, 0, 0]} opacity={0.6} />
                ...
                <tbody>
                  {report.days.map((d) => (
                    <tr key={d.date} className="border-b border-muted/50">
                      ...
                      <td className="text-right py-1">{fmtDollars(d.forecasted)}</td>
                      <td className="text-right py-1">{fmtDollars(d.actual)}</td>
                      <td className={`text-right py-1 ${
                        d.percentageError <= 10 ? 'text-green-600' :
                        d.percentageError <= 20 ? 'text-amber-600' : 'text-red-600'
                      }`}>
                        {d.actual > 0 ? `${d.percentageError.toFixed(0)}%` : '—'}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {trendData.length >= 2 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <p className="text-xs font-medium text-muted-foreground">Accuracy Trend (MAPE)</p>
                  {isTrendImproving !== null && (
                    <span className={`inline-flex items-center gap-1 text-xs ${
                      isTrendImproving ? 'text-green-600' : 'text-red-600'
                    }`}>
                      {isTrendImproving ? <TrendingDown className="h-3 w-3" /> : <TrendingUp className="h-3 w-3" />}
