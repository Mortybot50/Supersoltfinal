...
export function ShiftTemplateDialog({ open, onOpenChange, onApply }: ShiftTemplateDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Save and reuse common shift patterns</DialogDescription>
        </DialogHeader>
                ...
                <div key={t.id} className="flex items-center gap-3 p-3 rounded-lg border hover:bg-gray-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-400">
                      {t.start_time}–{t.end_time} • {t.break_minutes}min break • <span className="capitalize">{t.role}</span>
                    </div>
                  </div>
                  {onApply && (
                    <Button size="sm" variant="outline" className="h-7 text-xs shrink-0" onClick={() => { onApply(t); onOpenChange(false) }}>
                      Apply
                    </Button>
                  )}
                </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <Input type="number" value={breakMins} onChange={e => setBreakMins(e.target.value)} className="mt-1 h-8 text-sm" />
              </div>
            </div>
            <div>
