          ...
          <tr className="bg-white dark:bg-gray-800 border-b">
            ...
            {dates.map((date) => {
              ...
              return (
                ...
                >
                  <div
                    className={`font-medium ${compact ? "text-xs" : "text-sm"} ${isToday ? "text-blue-600" : ""}`}
                  >
                    {compact
                      ? format(date, "EEE d")
                  ...
                  <div className="text-xs text-muted-foreground">
                    {dayTotals.count > 0
                      ? `${dayTotals.hours.toFixed(1)}h · ${formatLabourCost(dayTotals.cost)}`
                      : "\u2014"}
                  </div>
                  {holiday && (
                    <Badge
                  ...
                  {isWeekend && !holiday && (
                    <Badge
                      variant="outline"
                      className="mt-1 text-[9px] bg-orange-100 text-orange-700 border-orange-200"
                    >
            ...
            <tr>
              ...
              >
                ...
                <p className="text-sm mt-2">
                  Add staff in the People section to start rostering
                </p>
              </td>
            </tr>
          ) : (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-3 h-3 rounded ${getRoleColor(group.label).bg}`}
                        />
                        {group.label}
                ...
                {group.staff.map((staffMember) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              <div
                                className={`w-2 h-2 rounded-full ${getRoleColor(staffMember.role).bg} shrink-0`}
                              />
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span
                                className={`text-xs ${overtimeWarning ? "text-red-600 font-semibold" : "text-muted-foreground"}`}
                              >
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() =>
                                  onAddAvailability(staffMember.id)
                                }
                              >
                      ...
                      {dates.map((date) => {
                        const shiftsOnDate = getShiftsForStaffOnDate(
                          staffMember.id,
                          date,
                        );
                        const availability = getAvailabilityForStaffOnDate(
                          staffMember.id,
                          date,
                        );
                        const holiday = isPublicHoliday(date);
                        ...
                        return (
                          <td
                            ...
                            onClick={() =>
                              !hasShifts &&
                              !isUnavailable &&
                              onAddShift(date, staffMember.id)
                            }
                          >
                      ...
                      {!compact && (
                        <td className="p-2 border-r bg-slate-50/50 dark:bg-slate-800/50 align-top">
                          <div className="text-xs text-center space-y-1">
                            <div
                              className={`font-semibold ${overtimeWarning ? "text-red-600" : ""}`}
                            >
                              {weeklyHours.toFixed(1)}h
                            </div>
              ...
              return (
                ...
                >
                  <div className="text-xs font-semibold">
                    {dayTotals.count} shifts
                  </div>
                  <div className="text-[10px] text-muted-foreground">
                    {dayTotals.hours.toFixed(1)}h
                  </div>
                  <div className="text-[10px] font-medium">
                    {formatLabourCost(dayTotals.cost)}
                  </div>
                </td>
              );
            })}
            {!compact && (
              <td className="p-2 text-center border-r bg-slate-100 dark:bg-slate-800">
                <div className="text-xs font-bold">
                  {dates
                    .reduce((sum, d) => sum + getDayTotals(d).hours, 0)
                    .toFixed(1)}
                  h
                </div>
                <div className="text-[10px] font-semibold">
                  {formatLabourCost(
                    dates.reduce((sum, d) => sum + getDayTotals(d).cost, 0),
                  )}
                </div>
              </td>
            )}
