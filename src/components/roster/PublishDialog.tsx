...
export function PublishDialog({ open, onOpenChange }: PublishDialogProps) {
  const { shifts, availability, selectedDate, publishRoster, staff } =
    useRosterStore();
  ...
  const shiftsToPublish =
    publishMode === "all" ? [...draftShifts, ...modifiedShifts] : draftShifts;
  ...
  const budget = useMemo(
    () => getLaborBudgetForWeek(selectedDate),
    [selectedDate, getLaborBudgetForWeek],
  );
  ...
  const budgetStatus = useMemo(
    () => calculateBudgetVariance(budget, metrics.totalCost),
    [budget, metrics.totalCost],
  );
  ...
  const isBudgetHardBlock =
    budgetStatus.variancePercent > 115 && !overBudgetReason.trim();

  const canProceedStep3 =
    criticalWarnings.length === 0 || complianceAcknowledged;
  const canPublish =
    shiftsToPublish.length > 0 && !isBudgetHardBlock && canProceedStep3;
          ...
          <div className="space-y-4">
            ...
            >
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    ...
                    {draftShifts.length} new
                    {modifiedShifts.length > 0
                      ? `, ${modifiedShifts.length} modified`
                      : ""}
                    )
                  </p>
                </div>
              ...
              >
                ...
                <div className="flex-1">
                  <Label
                    htmlFor="mode-changes"
                    className="font-medium cursor-pointer"
                  >
                    Publish Changes Only
                  </Label>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {draftShifts.length} new draft shift
                    {draftShifts.length !== 1 ? "s" : ""} (excludes
                    already-published)
                  </p>
                </div>
              </div>
            ...
            <div className="rounded-lg bg-gray-50 border p-3 text-sm text-gray-600">
              ...
              <span className="font-semibold text-gray-900">
                {shiftsToPublish.length} shift
                {shiftsToPublish.length !== 1 ? "s" : ""}
              </span>{" "}
              for{" "}
              <span className="font-semibold text-gray-900">
                {staffToNotify.length} staff member
                {staffToNotify.length !== 1 ? "s" : ""}
              </span>
              {confirmedShifts.length > 0 && (
                <span className="text-gray-400">
                  {" "}
                  · {confirmedShifts.length} already published
                </span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            {!budget ? (
              <div className="rounded-lg bg-gray-50 border p-4 text-center text-sm text-gray-500">
                ...
                <span className="text-gray-400">
                  Budget targets can be configured in Labour settings.
                </span>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-3 gap-3">
                  <div className="rounded-lg bg-gray-50 border p-3 text-center">
                    ...
                    <div className="text-lg font-bold">
                      {formatCurrency(budgetStatus.budgeted / 100)}
                    </div>
                  </div>
                  <div className="rounded-lg bg-gray-50 border p-3 text-center">
                    <div className="text-xs text-gray-400 mb-1">Actual</div>
                    <div className="text-lg font-bold">
                      {formatCurrency(budgetStatus.actual / 100)}
                    </div>
                  </div>
                  <div
                    className={cn(
                ...
                {budgetStatus.variancePercent > 105 &&
                  budgetStatus.variancePercent <= 115 && (
                    <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 space-y-2">
                      ...
                      <p className="text-xs text-amber-600">
                        Labour cost ({formatCurrency(budgetStatus.actual / 100)}
                        ) exceeds target (
                        {formatCurrency(budgetStatus.budgeted / 100)}). Review
                        shift costs or adjust the budget.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="rounded-lg bg-red-50 border border-red-200 p-3 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-red-700">
                        ...
                        {budgetStatus.variancePercent - 100}% above target)
                      </span>
                    </div>
                    <p className="text-xs text-red-600">
                      Labour cost is {budgetStatus.variancePercent}% of budget.
              ...
              <div className="flex items-center gap-2 rounded-lg bg-green-50 border border-green-200 p-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-600">
                    All shifts are within Fair Work guidelines.
                  </p>
                </div>
              </div>
            ) : (
                ...
                {criticalWarnings.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs font-semibold text-red-600 uppercase tracking-wide">
                      Critical — {criticalWarnings.length} issue
                      {criticalWarnings.length !== 1 ? "s" : ""}
                    </p>
                    {criticalWarnings.map((w) => (
                      <div
                        key={w.id}
                        className="rounded-md bg-red-50 border border-red-200 px-3 py-2 flex gap-2"
                      >
                ...
                {standardWarnings.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {standardWarnings.map((w) => (
                      <div
                        key={w.id}
                        className="rounded-md bg-amber-50 border border-amber-200 px-3 py-2 flex gap-2"
                      >
                ...
                {criticalWarnings.length > 0 && (
                  <div className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3 mt-2">
                    <Checkbox
                      ...
                      onCheckedChange={(c) =>
                        setComplianceAcknowledged(c === true)
                      }
                      className="mt-0.5"
                    />
                    <Label
                      htmlFor="ack-compliance"
                      className="text-sm text-red-700 cursor-pointer"
                    >
                      I acknowledge these compliance issues and wish to publish
                      anyway.
                    </Label>
                  </div>
                )}
                        ...
                        <tbody>
                          {dayShifts.map((s, idx) => (
                            ...
                            >
                              ...
                              <td className="px-2 py-1.5 text-gray-500 capitalize">
                                {s.role}
                              </td>
            ...
            <div className="rounded-lg bg-teal-50 border border-teal-200 p-3 flex items-center gap-2">
              ...
              <span className="text-sm text-teal-700">
                <span className="font-semibold">
                  {staffToNotify.length + noShiftStaff.length}
                </span>{" "}
            ...
            >
              {isPublishing ? (
                ...
                <>
                  ...
                  Publish {shiftsToPublish.length} shift
                  {shiftsToPublish.length !== 1 ? "s" : ""}
                </>
              )}
            </Button>
