...
export function ShiftContextMenu({ shift, onSeeCosts }: ShiftContextMenuProps) {
  const { copiedShift, setCopiedShift, addShift, updateShift, deleteShift } =
    useRosterStore();
    ...
    <>
      <DropdownMenu>
        ...
        >
          ...
          <DropdownMenuItem
            onClick={() => {
              /* open leave dialog pre-filled — see docs/TODO.md */
            }}
          >
      ...
      <Dialog open={showBreakEditor} onOpenChange={setShowBreakEditor}>
        <DialogContent className="sm:max-w-xs">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowBreakEditor(false)}>
              Cancel
            </Button>
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <DialogContent className="sm:max-w-xs">
          ...
          <p className="text-sm text-gray-600 py-2">
            Delete this shift for <strong>{shift.staff_name}</strong>? This
            cannot be undone.
          </p>
          <DialogFooter>
            <Button
