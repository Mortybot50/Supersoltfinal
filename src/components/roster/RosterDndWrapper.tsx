  ...
  try {
    ...
    for (const qtId of requiredQualTypeIds) {
      const qual = (staffQuals || []).find(
        (q) => q.qualification_type_id === qtId,
      );
      const qtName =
        (qualTypes || []).find((qt) => qt.id === qtId)?.name || "Unknown";
...
export function RosterDndWrapper({ children }: { children: ReactNode }) {
  const { staff, shifts, moveShift, addShift, pendingShift, setPendingShift } =
    useRosterStore();
      ...
      if (activeData?.type === DRAGGABLE_SHIFT_TYPE) {
        ...
        const shiftDate =
          shift.date instanceof Date
            ? shift.date
            : parse(String(shift.date).slice(0, 10), "yyyy-MM-dd", new Date());
        if (
          shift.staff_id === targetStaffId &&
          isSameDay(shiftDate, targetDate)
        ) {
          return;
        }
    ...
    (config: ShiftConfig) => {
      ...
      try {
        ...
        addShift({
          ...
          penalty_type:
            (breakdown.penalty_type as RosterShift["penalty_type"]) || "none",
  ...
  const handleShiftConfirm = useCallback(
    async (config: ShiftConfig) => {
      const currentPending = useRosterStore.getState().pendingShift;
      if (!currentPending) return;

      try {
        const expiredQuals = await checkExpiredQualifications(
          currentPending.staffId,
          config.role,
        );
  ...
  return (
    <>
      ...
      <AlertDialog
        open={!!qualWarning}
        onOpenChange={(open) => !open && handleQualCancel()}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  <strong>{pendingShift?.staffName}</strong> has qualifications
                  that are expired or expiring soon for the{" "}
                  <strong>{qualWarning?.pendingConfig.role}</strong> role.
                </p>
                  ...
                  {qualWarning?.expiredQuals.map((q) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant={
                            q.status === "expired" ? "destructive" : "outline"
                          }
                          className={
                            q.status === "expiring"
                ...
                <p className="text-xs text-muted-foreground">
                  You can override and save the shift anyway, or cancel to
                  update qualifications first.
                </p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleQualCancel}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
