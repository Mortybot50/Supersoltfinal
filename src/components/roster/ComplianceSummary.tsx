...
export function ComplianceSummary({ onClose }: ComplianceSummaryProps) {
  ...
  const warnings = useMemo(() => getAllRosterWarnings(shifts, availability), [shifts, availability])
  ...
  if (warnings.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center px-4">
        ...
        <p className="text-xs text-gray-400 mt-0.5">No compliance issues for this week</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b bg-white sticky top-0 z-10">
        <div className="flex items-center gap-2">
          ...
          {errors.length > 0 && (
            <Badge variant="destructive" className="text-[10px] h-4 px-1">{errors.length} errors</Badge>
          )}
        ...
        {onClose && (
          <button onClick={onClose} className="h-5 w-5 flex items-center justify-center rounded hover:bg-gray-100">
        ...
        {errors.length > 0 && (
          <div className="border-b">
            <div className="px-3 py-1.5 bg-red-50">
              <span className="text-[10px] font-semibold text-red-600 uppercase tracking-wide">Errors — must fix</span>
            </div>
            {errors.map(w => (
        ...
        {warningsOnly.length > 0 && (
          <div>
            <div className="px-3 py-1.5 bg-orange-50">
              <span className="text-[10px] font-semibold text-orange-600 uppercase tracking-wide">Warnings</span>
            </div>
            {warningsOnly.map(w => (
              <WarningRow key={w.id} warning={w} />
            ))}
    ...
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          <span className="text-xs font-medium truncate">{warning.staff_name}</span>
          <Badge variant="outline" className="text-[9px] h-3.5 px-1 border-gray-200 text-gray-500 shrink-0">
            {typeLabel}
          </Badge>
        </div>
        <p className="text-[11px] text-gray-500 leading-tight">{warning.message}</p>
      </div>
    </div>
  )
}
