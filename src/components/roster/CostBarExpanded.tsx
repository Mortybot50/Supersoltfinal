...
export function CostBarExpanded() {
  ...
  return (
    <div className="bg-white border-t border-gray-100 px-4 py-3 space-y-3 max-h-72 overflow-y-auto print:hidden">
      ...
      <div className={cn('flex items-center gap-3 px-3 py-2 rounded-lg border', statusColors[costStatus])}>
        <TrendingUp className="h-4 w-4 shrink-0" />
        <div className="flex-1">
          <div className="text-sm font-semibold">
            Estimated labour cost: {formatCurrency(weeklyLabourCostCents)}
            {labourPercent !== null && (
              <span className="ml-1">({labourPercent.toFixed(1)}% of avg revenue)</span>
            )}
          </div>
          {hasRevenueData && avgWeeklyRevenueCents ? (
            <div className="text-[10px] opacity-70">
              Based on avg weekly revenue of {formatCurrency(avgWeeklyRevenueCents)} (last 4 weeks POS data)
            </div>
          ) : (
            <div className="text-[10px] opacity-70">
      ...
      <div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {dailyBreakdown.map((day) => {
            const maxCost = Math.max(...dailyBreakdown.map(d => d.totalCostCents), 1)
            const barHeight = day.totalCostCents > 0 ? Math.max(8, (day.totalCostCents / maxCost) * 40) : 0
            const hasPenalty = day.penaltyCostCents > 0

            return (
              <div key={day.dateStr} className="flex flex-col items-center gap-0.5">
                <span className="text-[10px] font-medium text-gray-500">{day.dayLabel}</span>
                ...
                <span className="text-[10px] font-semibold tabular-nums">
                  {day.totalCostCents > 0 ? formatCurrency(day.totalCostCents) : '—'}
                </span>
