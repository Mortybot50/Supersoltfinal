      ...
      .filter((shift) => {
        switch (spotlightFilter) {
          case "validation_errors":
            return !shift.warnings || shift.warnings.length === 0;
          case "overtime":
            return (weeklyHours[shift.staff_id] || 0) <= 38;
          case "open_vacant":
            return !shift.is_open_shift;
        ...
        <div style={gridStyle} className="min-w-max">
          ...
          {roleGroups.map(([groupKey, groupStaff]) => {
            ...
            const groupShifts = shifts.filter((s) =>
              groupStaff.some((m) => m.id === s.staff_id),
            );
                ...
                {isExpanded &&
                  groupStaff.map((member) => (
                    <RosterRow
                      ...
                      ghostShifts={ghostShifts.filter(
                        (s) => s.staff_id === member.id,
                      )}
