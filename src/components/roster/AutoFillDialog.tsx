...
export function AutoFillDialog({ open, onOpenChange }: AutoFillDialogProps) {
  const { ghostShifts, shifts, staff, availability, selectedDate, addShift } = useRosterStore()
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      toast.success(`Auto-filled ${preview.length} shift${preview.length > 1 ? 's' : ''} from last week`)
      onOpenChange(false)
      setPreview(null)
    } finally {
      setIsRunning(false)
    ...
    <Dialog open={open} onOpenChange={v => { onOpenChange(v); if (!v) setPreview(null) }}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Copy last week's shifts to this week, skipping staff who are unavailable
            ...
            <div className="space-y-2">
              <div className="rounded-lg bg-purple-50 border border-purple-200 p-3">
                <p className="text-sm text-purple-700">
                  Found <strong>{ghostShifts.length}</strong> shifts from last week.
                  Auto-fill will:
                </p>
              ...
              {ghostShifts.length === 0 && (
                <p className="text-xs text-gray-400 text-center">No shifts from last week to copy</p>
              )}
            </div>
          ) : (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">
                  {preview.length} shift{preview.length !== 1 ? 's' : ''} to create
                </span>
              ...
              {preview.length === 0 ? (
                ...
                <div className="max-h-48 overflow-y-auto space-y-1.5">
                  {preview.map((s, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-2 text-xs px-2 py-1.5 rounded border bg-gray-50">
                        <Check className="h-3.5 w-3.5 text-green-500 shrink-0" />
                        <span className="font-medium w-16 shrink-0">{format(d, 'EEE d MMM')}</span>
                        <span className="truncate">{member?.name || s.staff_name}</span>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => { onOpenChange(false); setPreview(null) }}>
            Cancel
          </Button>
