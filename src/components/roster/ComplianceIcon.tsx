...
export function ComplianceIcon({ shiftId, className }: ComplianceIconProps) {
  ...
  const warnings = useMemo(() => {
    ...
    if (shift.warnings) {
      shift.warnings.forEach(w => all.push({ severity: 'warning', message: w }))
    }

    // Rest gap check for this staff member
    const staffShifts = shifts.filter(s => s.staff_id === shift.staff_id)
    const restWarnings = detectRestGapWarnings(staffShifts).filter(w => w.shift_id === shiftId)
    restWarnings.forEach(w => all.push({ severity: w.severity as 'warning' | 'error', message: w.message }))

    // Break warnings
    const breakWarnings = detectBreakWarnings([shift])
    breakWarnings.forEach(w => all.push({ severity: w.severity as 'warning' | 'error', message: w.message }))
    ...
    <Tooltip>
      ...
      <TooltipContent side="top" className="max-w-xs">
        <ul className="space-y-0.5">
          {warnings.map((w, i) => (
            <li key={i} className={cn('text-xs', w.severity === 'error' ? 'text-red-200' : 'text-orange-200')}>
              {w.message}
            </li>
          ))}
        </ul>
