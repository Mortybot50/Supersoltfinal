...
import { formatTimeCompact, getWeekStart } from '@/lib/utils/rosterCalculations'
...
export function QuickBuildSidebar() {
  const {
    quickBuildOpen, setQuickBuildOpen,
    templates, selectedDate,
    rosterPatterns, staff,
    copyWeekShifts, copyDayShifts,
    autoFill, saveCurrentAsTemplate,
    applyRosterTemplate, addRosterPattern,
    updateRosterPattern, deleteRosterPattern,
      ...
      >
        <SheetHeader className="px-4 py-3 border-b shrink-0">
          ...
          <SheetDescription className="text-xs">
            Bulk roster construction — {format(selectedDate, 'd MMM')}–{format(addDays(selectedDate, 6), 'd MMM yyyy')}
          </SheetDescription>
        </SheetHeader>

        <div className="flex-1 overflow-y-auto">
          <Accordion type="multiple" defaultValue={['templates', 'copy']} className="w-full">
...
interface TemplatesSectionProps {
  ...
  onApply: (t: ReturnType<typeof useRosterStore>['templates'][0]) => Promise<{ created: number; conflicts: number }>
  onSaveCurrent: (name: string) => Promise<boolean>
}

function TemplatesSection({ templates, selectedDate, shifts, onApply, onSaveCurrent }: TemplatesSectionProps) {
  ...
  const handleSave = async () => {
    ...
    if (ok) { setSaveName(''); setShowSaveForm(false) }
    setIsSaving(false)
  }
  ...
  const handleApply = async (t: typeof templates[0]) => {
    ...
    if (created > 0) {
      toast.success(`Applied "${t.name}": ${created} shift${created !== 1 ? 's' : ''} created${conflicts > 0 ? `, ${conflicts} skipped (conflicts)` : ''}`)
    } else if (conflicts > 0) {
      toast.warning(`All ${conflicts} shifts skipped — conflicts with existing schedule`)
    } else {
  ...
  const templateShiftCount = (t: typeof templates[0]) => {
    if (t.template_shifts && t.template_shifts.length > 0) return t.template_shifts.length
    return t.days_of_week.length
  }

  const templateHours = (t: typeof templates[0]) => {
    const shifts = t.template_shifts && t.template_shifts.length > 0
      ? t.template_shifts
      : t.days_of_week.map(() => ({ start_time: t.start_time, end_time: t.end_time, break_minutes: t.break_minutes }))
    ...
    <AccordionItem value="templates" className="border-b">
      <AccordionTrigger className="px-4 py-3 text-sm font-medium hover:no-underline">
        <div className="flex items-center gap-2">
          ...
          Templates
          {templates.length > 0 && (
            <Badge variant="secondary" className="text-xs px-1.5 py-0 h-5">{templates.length}</Badge>
          )}
        </div>
      </AccordionTrigger>
      <AccordionContent className="px-4 pb-4 pt-1 space-y-3">
        ...
        {!showSaveForm ? (
          ...
          >
            ...
            {weekShiftCount > 0 && <span className="text-gray-400">({weekShiftCount} shifts)</span>}
          </Button>
        ) : (
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" className="h-8 text-xs px-3" onClick={handleSave} disabled={isSaving || !saveName.trim()}>
              {isSaving ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
            </Button>
            <Button size="sm" variant="ghost" className="h-8 text-xs px-2" onClick={() => { setShowSaveForm(false); setSaveName('') }}>
              ✕
            </Button>
          </div>
        )}
        ...
        {templates.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {templates.map(t => (
              <div key={t.id} className="flex items-center gap-2 p-2.5 rounded-lg border bg-gray-50 hover:bg-gray-100 transition-colors">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{t.name}</div>
                  <div className="text-xs text-gray-400 mt-0.5">
                    {templateShiftCount(t)} shift{templateShiftCount(t) !== 1 ? 's' : ''}
                    {' · '}
        ...
        <Dialog open={!!confirmTemplate} onOpenChange={v => !v && setConfirmTemplate(null)}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle>Apply Template</DialogTitle>
              <DialogDescription>
                This will create <strong>{confirmTemplate ? templateShiftCount(confirmTemplate) : 0}</strong> shifts in the current week.
                Conflicts with existing shifts will be skipped.
              </DialogDescription>
            </DialogHeader>
            <div className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">
              ...
              <span className="text-xs text-gray-400">
                {confirmTemplate ? `${templateShiftCount(confirmTemplate)} shifts · ${templateHours(confirmTemplate).toFixed(1)}h total` : ''}
              </span>
            </div>
            <DialogFooter>
              ...
              <Button
                ...
                disabled={!!applyingId}
              >
                {applyingId ? <RefreshCw className="h-4 w-4 animate-spin mr-2" /> : null}
                Apply Template
              </Button>
            </DialogFooter>
          </DialogContent>
...
interface CopyShiftsSectionProps {
  selectedDate: Date
  onCopyWeek: (sourceWeekStart: Date) => Promise<{ created: number; conflicts: number }>
  onCopyDay: (sourceDate: Date, targetDates: Date[]) => Promise<{ created: number; conflicts: number }>
}

function CopyShiftsSection({ selectedDate, onCopyWeek, onCopyDay }: CopyShiftsSectionProps) {
  ...
  const handleCopyLastWeek = async () => {
    ...
    if (created === 0 && conflicts === 0) {
      ...
      toast.success(`Copied ${created} shift${created !== 1 ? 's' : ''}${conflicts > 0 ? `, ${conflicts} skipped (conflicts)` : ''}`)
    }
  }

  const handleCopySpecificWeek = async () => {
    if (!specificWeekDate) { toast.error('Select a source week'); return }
  ...
  const handleCopyDay = async () => {
    if (!sourceDayDate) { toast.error('Select a source day'); return }
    if (targetDays.length === 0) { toast.error('Select at least one target day'); return }
  ...
  return (
    <AccordionItem value="copy" className="border-b">
      ...
      <AccordionContent className="px-4 pb-4 pt-1 space-y-4">
        ...
        {/* Copy from last week */}
        <div className="space-y-1.5">
          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">Last Week</div>
          <div className="flex items-center justify-between p-2.5 rounded-lg border bg-gray-50">
            <div>
              ...
              <div className="text-xs text-gray-400">
                {format(prevWeekStart, 'd MMM')}–{format(addDays(prevWeekStart, 6), 'd MMM')}
              </div>
            </div>
            <Button
              size="sm"
        ...
        <div className="space-y-1.5">
          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">Specific Week</div>
          <div className="flex gap-2">
            <Input
              type="date"
          ...
          <p className="text-xs text-gray-400">Select any day within the source week.</p>
        </div>

        {/* Copy single day */}
        <div className="space-y-2">
          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">Single Day</div>
          <div>
          ...
          <div>
            <Label className="text-xs text-gray-500">Copy to (current week)</Label>
          ...
          {sourceDayDate && targetDays.length > 0 && (
            <p className="text-xs text-blue-600">
              Will copy {format(new Date(sourceDayDate + 'T00:00:00'), 'EEE d MMM')} → {targetDays.length} day{targetDays.length !== 1 ? 's' : ''}
            </p>
          )}
          <Button
            size="sm"
...
interface PatternsSectionProps {
  ...
  venueId: string
  orgId: string
  onAdd: (p: Parameters<ReturnType<typeof useRosterStore>['addRosterPattern']>[0]) => Promise<boolean>
...
function PatternsSection({ patterns, staff, venueId, orgId, onAdd, onUpdate, onDelete, onApply }: PatternsSectionProps) {
  ...
  const handleApply = async (id: string, name: string) => {
    ...
    setApplyingId(null)
    if (created > 0) {
      toast.success(`Pattern "${name}": ${created} shift${created !== 1 ? 's' : ''} created${conflicts > 0 ? `, ${conflicts} skipped` : ''}`)
    } else if (conflicts > 0) {
    ...
    <AccordionItem value="patterns" className="border-b">
      <AccordionTrigger className="px-4 py-3 text-sm font-medium hover:no-underline">
        <div className="flex items-center gap-2">
          ...
          Roster Patterns
          {patterns.length > 0 && (
            <Badge variant="secondary" className="text-xs px-1.5 py-0 h-5">{patterns.length}</Badge>
          )}
        </div>
      </AccordionTrigger>
      <AccordionContent className="px-4 pb-4 pt-1 space-y-3">
        <p className="text-xs text-gray-500">
          Recurring shift patterns applied each week (e.g., "Mon–Wed–Fri Kitchen Open").
        </p>
                ...
                <div className="flex items-center gap-2 p-2.5">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">
                      ...
                      {p.shifts.length > 0 && (
                        <> · {[...new Set(p.shifts.map(s => DAY_NAMES[s.day_of_week]))].join(', ')}</>
                      )}
                    </div>
                  </div>
                  <div className="flex gap-1 shrink-0">
                    <Button
                      ...
                      title="Edit pattern"
                    >
                      <ChevronRight className={cn('h-3.5 w-3.5 transition-transform', editingId === p.id && 'rotate-90')} />
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
...
interface PatternEditorProps {
  ...
  staff: ReturnType<typeof useRosterStore>['staff']
  onSave: (updates: { name?: string; shifts?: TemplateShiftDef[] }) => Promise<boolean>
  onClose: () => void
}

function PatternEditor({ pattern, staff, onSave, onClose }: PatternEditorProps) {
  const [name, setName] = useState(pattern.name)
  const [shifts, setShifts] = useState<TemplateShiftDef[]>(pattern.shifts)
  const [isSaving, setIsSaving] = useState(false)

  const addShift = () => {
    setShifts(prev => [...prev, { day_of_week: 1, start_time: '09:00', end_time: '17:00', break_minutes: 30, role: 'crew', staff_id: null }])
  }
  ...
  const updateShift = (i: number, updates: Partial<TemplateShiftDef>) => {
    setShifts(prev => prev.map((s, idx) => idx === i ? { ...s, ...updates } : s))
  }
  ...
  const handleSave = async () => {
    ...
    setIsSaving(false)
    if (ok) { toast.success('Pattern updated'); onClose() }
  }
  ...
  return (
    <div className="border-t px-3 pb-3 pt-2 space-y-2">
      <div>
        ...
        <Input value={name} onChange={e => setName(e.target.value)} className="mt-1 h-7 text-xs" />
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <Button variant="ghost" size="sm" className="w-full h-7 text-xs gap-1" onClick={addShift}>
      ...
      <div className="flex gap-2 justify-end pt-1">
        ...
        <Button size="sm" className="h-7 text-xs" onClick={handleSave} disabled={isSaving || !name.trim()}>
          {isSaving ? <RefreshCw className="h-3 w-3 animate-spin mr-1" /> : null}
          Save
        </Button>
      </div>
    </div>
...
interface NewPatternFormProps {
  staff: ReturnType<typeof useRosterStore>['staff']
  venueId: string
  onSave: (pattern: Parameters<ReturnType<typeof useRosterStore>['addRosterPattern']>[0]) => Promise<void>
  onCancel: () => void
}

function NewPatternForm({ staff, venueId, onSave, onCancel }: NewPatternFormProps) {
  const [name, setName] = useState('')
  const [shifts, setShifts] = useState<TemplateShiftDef[]>([
    { day_of_week: 1, start_time: '09:00', end_time: '17:00', break_minutes: 30, role: 'crew', staff_id: null },
  ])
  const [isSaving, setIsSaving] = useState(false)
  ...
  const handleSave = async () => {
    if (!name.trim()) { toast.error('Enter a pattern name'); return }
    if (shifts.length === 0) { toast.error('Add at least one shift'); return }
    setIsSaving(true)
    await onSave({ organization_id: orgId, venue_id: venueId, name: name.trim(), shifts, is_active: true })
    setIsSaving(false)
  }
...
function ShiftDefRow({ shiftDef, staff, onChange, onRemove }: ShiftDefRowProps) {
  return (
    <div className="grid grid-cols-[80px_1fr_1fr_1fr_auto] gap-1 items-center">
      ...
      {/* Start time */}
      <Input type="time" value={shiftDef.start_time} onChange={e => onChange({ start_time: e.target.value })} className="h-7 text-xs px-1.5" />
      {/* End time */}
      <Input type="time" value={shiftDef.end_time} onChange={e => onChange({ end_time: e.target.value })} className="h-7 text-xs px-1.5" />
      {/* Role */}
      <select
        value={shiftDef.role}
      ...
      {/* Remove */}
      {onRemove ? (
        <Button variant="ghost" size="icon" className="h-7 w-7 text-red-400 hover:text-red-600 hover:bg-red-50 shrink-0" onClick={onRemove}>
...
interface AutoFillSectionProps {
  onAutoFill: (mode: 'copy_last' | 'assign_open' | 'build_empty') => Promise<void>
}
  ...
  const actions = [
    {
      ...
      description: 'Copy last week\'s roster, skip unavailable staff, flag gaps.',
    },
    {
      mode: 'assign_open' as const,
      icon: <Wand2 className="h-4 w-4 text-purple-500" />,
      title: 'Assign Staff to Open Shifts',
      description: 'Match available staff to open shifts. Prefers staff who worked the slot last week, then balances hours.',
    },
    {
      mode: 'build_empty' as const,
      icon: <AlertTriangle className="h-4 w-4 text-amber-500" />,
      title: 'Build Empty Roster',
      description: 'Generate shift slots from demand data — requires sales forecasting setup.',
    },
  ]
    ...
    <AccordionItem value="autofill" className="border-b">
      ...
      <AccordionContent className="px-4 pb-4 pt-1 space-y-2">
        ...
        {actions.map(({ mode, icon, title, description }) => (
          <div key={mode} className="flex gap-3 p-3 rounded-lg border hover:bg-gray-50 transition-colors">
