...
export function SalesForecastOverlay({
  dates,
  className,
}: SalesForecastOverlayProps) {
  ...
  return (
    <div
      className={cn("flex gap-0 pointer-events-none select-none", className)}
    >
      {forecast.map((f, i) => (
        <div
          key={i}
...
export function PrepLoadBadge({
  ...
  const load = useMemo(() => {
    ...
    const avg =
      weekRevenues.reduce((s, r) => s + r, 0) / (weekRevenues.length || 1);
    ...
    if (ratio < 0.7)
      return { label: "Quiet", color: "bg-green-100 text-green-700" };
    return null;
  }, [orders, date, dates]);
