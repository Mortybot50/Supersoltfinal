  ...
  const onMouseDown = useCallback(
    (type: "start" | "end") => (e: React.MouseEvent) => {
      ...
      const onMouseMove = (ev: MouseEvent) => {
        ...
        const [origH, origM] = dragging.current.originalTime.split(":").map(Number)
        let newMins = origH * 60 + origM + deltaMins
        // Clamp to operating hours
        newMins = Math.max(TIMELINE_START * 60, Math.min(TIMELINE_END * 60, newMins))
        ...
        } else if (dragging.current.type === "end" && newTime > shift.start_time) {
...
function ShiftBar({ shift, containerRef, onSelect, onUpdate, staffName, role }: ShiftBarProps) {
  const pos = getShiftPosition(shift.start_time, shift.end_time)
  const breakPos = getBreakPosition(shift.start_time, shift.end_time, shift.break_minutes)
  ...
  return (
    <div className="flex items-center h-10 hover:bg-gray-50 dark:hover:bg-gray-800/50">
      ...
      <div className="w-48 shrink-0 flex items-center gap-1.5 px-3 border-r">
        <Badge className={`${roleColor.bg} text-white text-[9px] px-1.5 py-0 h-5 font-bold shrink-0`}>
          {roleBadge}
        </Badge>
        <div className="flex flex-col min-w-0">
          <span className="text-xs font-medium truncate leading-tight">{staffName}</span>
  ...
  return (
    <div className="divide-y">
      ...
      {shifts.map((shift) => {
        ...
        const roleBadge = ROLE_BADGE_LABELS[shift.role?.toLowerCase() ?? ""] ?? "CRW"
        return (
          <button
            key={shift.id}
          ...
          >
            ...
            <div className="text-right shrink-0">
              <div className="text-sm font-medium">{shift.total_hours.toFixed(1)}h</div>
              <div className="text-xs text-muted-foreground">{formatLabourCost(shift.total_cost)}</div>
            </div>
          </button>
        )
      })}
...
export function DayView({ onSelectShift }: DayViewProps) {
      ...
      shifts
        .filter(
          (s) =>
            !s.is_open_shift &&
            s.status !== "cancelled" &&
            isSameDay(s.date instanceof Date ? s.date : new Date(s.date), selectedDate)
        )
  ...
  const { data: posSlots } = useDemandForecast(currentVenue?.id ?? null, selectedDate)
    ...
    <div className="flex-1 overflow-auto bg-white dark:bg-gray-800">
      ...
      <div className="border-b px-4 py-2 flex items-center gap-6 bg-gray-50 dark:bg-gray-900 text-sm">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-semibold tabular-nums">{formatHours(dayStats.totalHours)}</span>
        </div>
        {posSlots && (
          <>
            <div className="flex items-center gap-1.5">
              <span className="text-muted-foreground text-xs">Recommended</span>
              <span className="font-semibold tabular-nums">{formatHours(recommendedHours)}</span>
            </div>
            <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-semibold tabular-nums">{formatLabourCost(dayStats.totalCost)}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-muted-foreground text-xs">Staff</span>
          <span className="font-semibold tabular-nums">{dayStats.staffCount}</span>
        </div>
      </div>
