...
export function useOrders(filters?: UseOrdersFilters) {
  const { venueId, startDate, endDate, channel, source, includeVoids = false } = filters || {}

  return useQuery({
    queryKey: ['orders', venueId, startDate, endDate, channel, source, includeVoids],
    queryFn: async (): Promise<OrderRow[]> => {
      let query = supabase
        .from('orders')
        .select('id, venue_id, order_number, order_datetime, channel, gross_amount, tax_amount, net_amount, discount_amount, tip_amount, service_charge, is_void, is_refund, refund_reason, payment_method, source, external_id, customer_name, staff_member, notes')
