...
interface InvoiceIntakeState {
  ...
  updateLineMapping: (
    jobId: string,
    lineIndex: number,
    mapping: Partial<InvoiceIntakeJob["mapping_json"][0]>,
  ) => void;
...
export const useInvoiceIntakeStore = create<InvoiceIntakeState>((set, get) => ({
    ...
    try {
      ...
      set((state) => ({
        jobs: state.jobs.map((job) =>
          job.id === jobId
            ? { ...job, status: "parsing" as InvoiceIntakeStatus }
            : job,
        ),
      }));
      ...
      if (duplicate && duplicate.status === "approved") {
        ...
        alert(
          `Duplicate invoice detected! This invoice has already been processed.\n\nExisting Job ID: ${duplicate.id}`,
        );
        return;
      }
      ...
      set((state) => ({
        jobs: state.jobs.map((job) =>
          job.id === jobId
            ? { ...job, status: "failed" as InvoiceIntakeStatus }
            : job,
        ),
      }));
    ...
    try {
      ...
      const mappings = job.lines_json.map((line, index) => {
        ...
        const unitCostCents = Math.round(
          (line.unit_price / packToUnitFactor) * 100,
        );
  ...
  approveInvoice: async (jobId: string, notes?: string) => {
    ...
    try {
      ...
      const lineTotal = job.lines_json.reduce(
      ...
      if (variancePercent > 2) {
        alert(
          `Cannot approve: Totals mismatch by ${variancePercent.toFixed(1)}%`,
        );
        set({ isProcessing: false });
        return;
      }
      ...
      for (let i = 0; i < job.mapping_json.length; i++) {
        ...
        if (!mapping.ingredient_id || mapping.match_type === "new_item")
          continue;
        ...
        const ingredient = currentIngredients.find(
          (ing) => ing.id === mapping.ingredient_id,
        );
      ...
      set((state) => ({
        jobs: state.jobs.map((j) =>
          j.id === jobId
            ? {
                ...j,
                status: "approved" as InvoiceIntakeStatus,
                updated_at: new Date(),
              }
            : j,
        ),
      ...
      alert(
        `Invoice approved! ${job.lines_json.length} items processed.${costUpdates > 0 ? ` ${costUpdates} ingredient cost(s) updated.` : ""}`,
      );
      ...
      set((state) => ({
        jobs: state.jobs.map((j) =>
          j.id === jobId
            ? {
                ...j,
                status: "rejected" as InvoiceIntakeStatus,
                updated_at: new Date(),
              }
            : j,
        ),
