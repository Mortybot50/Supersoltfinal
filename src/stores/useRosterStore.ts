...
function mapShiftRow(row: any): RosterShift {
  return {
    ...
    total_hours: calcHours(
      row.start_time,
      row.end_time,
      row.break_duration_mins ?? row.break_minutes ?? 0,
    ),
...
export const useRosterStore = create<RosterStore>((set, get) => ({
  ...
  init: async (venueId, orgId) => {
    ...
    try {
      const [staff, templates, availability, rosterPatterns] =
        await Promise.all([
          loadStaffFromDB(),
  ...
  navigateWeek: (direction) => {
    ...
    if (view === "day") {
      ...
      set({
        selectedDate: next,
        selectedDayIndex: next.getDay() === 0 ? 6 : next.getDay() - 1,
      });
  ...
  setSelectedDay: (date) => {
    set({
      selectedDate: date,
      selectedDayIndex: date.getDay() === 0 ? 6 : date.getDay() - 1,
    });
  },
  ...
  addShift: async (partial) => {
    ...
    set({
      ...
      openShifts: newShift.is_open_shift
        ? [...get().openShifts, newShift]
        : get().openShifts,
    });
  ...
  updateShift: async (id, updates) => {
    ...
    set({
      shifts: get().shifts.map((s) =>
        s.id === id ? { ...s, ...shiftUpdates } : s,
      ),
      openShifts: get().openShifts.map((s) =>
        s.id === id ? { ...s, ...shiftUpdates } : s,
      ),
    });
  ...
  deleteShift: async (id) => {
    ...
    set({
      ...
      selectedShiftId:
        get().selectedShiftId === id ? null : get().selectedShiftId,
    });
  ...
  loadWeek: async (weekStart) => {
    ...
    set({
      isLoading: true,
      ...(dateChanged ? { selectedDate: normalised } : {}),
    });
    ...
    try {
      ...
      const [curData, prevData] = await Promise.all([
        (supabase as any)
          .from("roster_shifts")
          .select(
            `*, staff!inner(id, org_members!inner(profiles!inner(first_name, last_name)))`,
          )
          .eq("venue_id", venueId)
      ...
      const mapWithName = (row: any): RosterShift => {
        ...
        return {
          ...mapShiftRow(row),
          staff_name: `${firstName} ${lastName}`.trim() || row.staff_name || "",
        };
      };
  ...
  publishRoster: async (weekStart, mode = "all") => {
    ...
    if (ok) {
      set({
        shifts: get().shifts.map((s) =>
          ids.includes(s.id) ? { ...s, status: "confirmed" } : s,
        ),
      });
  ...
  copyDayShifts: async (sourceDate, targetDates) => {
    ...
    if (!venueId || !orgId || targetDates.length === 0)
  ...
  autoFill: async (mode) => {
    ...
    if (mode === "copy_last") {
      ...
      toast.success(
        `Auto-filled ${created} shift${created !== 1 ? "s" : ""} from last week`,
      );
      return;
    }
    ...
    if (mode === "assign_open") {
      ...
      for (const openShift of openInWeek) {
        const shiftDate =
          openShift.date instanceof Date
            ? openShift.date
            : new Date(openShift.date);
        ...
        const ranked = [...eligible].sort((a, b) => {
          const aLast = ghostShifts.some((g) => {
            ...
            return (
              g.staff_id === a.id &&
              gDate.getDay() === dayOfWeek &&
              g.start_time === openShift.start_time
            );
          });
        ...
        const best = ranked[0];
        await updateShift(openShift.id, {
          staff_id: best.id,
          staff_name: best.name,
          is_open_shift: false,
        });
        staffHours[best.id] =
          (staffHours[best.id] || 0) + (openShift.total_hours || 0);
        assigned++;
      }

      if (assigned === 0) {
        toast.info(
          "No open shifts could be assigned — check staff availability and roles",
        );
      } else {
        toast.success(
          `Assigned ${assigned} open shift${assigned !== 1 ? "s" : ""}`,
        );
      }
      return;
    }
    ...
    if (mode === "build_empty") {
      toast.info(
        "Build Empty Roster requires demand data. Set up sales forecasting first.",
      );
    }
  },
  ...
  saveCurrentAsTemplate: async (name) => {
    ...
    if (saved) {
      ...
      toast.success(
        `Template "${name}" saved with ${weekShifts.length} shifts`,
      );
      return true;
    }
    return false;
