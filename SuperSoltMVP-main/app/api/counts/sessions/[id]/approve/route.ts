...
// POST /api/counts/sessions/[id]/approve - Approve count session
export async function POST(
  ...
  try {
    ...
    // Verify session exists and is in SUBMITTED status
    ...
    if (session.status !== "SUBMITTED") {
      return NextResponse.json(
        { error: "Only SUBMITTED sessions can be approved" },
        { status: 400 }
      );
    }
