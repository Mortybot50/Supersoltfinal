...
export async function POST(req: Request) {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      const r = rows[i];
      const validation = SupplierRow.safeParse(r);
      ...
      if (!validation.success) {
        ...
        const errorMessages = Object.entries(fieldErrors)
          ...
          .join("; ");
        rowErrors.push({ row: i + 2, message: errorMessages }); // +2 for header and 0-index
        continue;
      }
    ...
    return NextResponse.json({
      orgId,
      parsed,
      errors: rowErrors.slice(0, 100), // Return first 100 errors
      totalErrors: rowErrors.length,
    });
  } catch (error: any) {
    console.error("Supplier preview error:", error);
